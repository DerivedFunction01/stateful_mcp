...
## Adapters aren't all the same kind of thing

Some adapters are **in-process**: sqlite3, file, memory, IndexedDB — they live in the same runtime as the MCP server, imported as a library, called as a function. The adapter is just a class that wraps the driver.

Some adapters are **cross-process or cross-runtime**: pandas runs in Python; a `pg` connection might be fine in Node but you might also want to call an existing Python data service. These aren't "adapters" in the same sense — they're process boundaries.

This matters because the adapter registry as designed assumes one runtime. `registerAdapter("sqlite", SqliteRepository)` only works if `SqliteRepository` is importable in the same process. You can't `import pandas from 'pandas'` in Node.

## The actual options for cross-runtime adapters

**Option A: Subprocess adapter** — the Node MCP server spawns a Python child process, communicates over stdin/stdout or a local socket. The Python side runs pandas; the Node side has a `PandasSubprocessEngine` adapter that serializes `QueryDefinition` to JSON, sends it, gets rows back. The adapter interface is identical from `FilterStore`'s perspective — it's just another `QueryEngine` — but internally it's IPC, not a function call. This is the cleanest architectural fit with the existing design: the interface boundary stays clean, the runtime boundary is entirely inside the adapter's implementation.

**Option B: Sidecar HTTP service** — the Python pandas logic runs as a separate HTTP server (FastAPI, Flask). The Node adapter calls it exactly like any other `remote_url` engine (adapter type 4 from the taxonomy). This is actually already designed — it's just an `HttpQueryEngine` where the "remote service" happens to be a local pandas process rather than an external API. No new adapter type needed.

**Option C: Rewrite the whole MCP server in Python** — only makes sense if pandas is the *primary* execution engine and SQL/memory are secondary, not the other way around.

## The pragmatic answer

**Node is the right host for the MCP server.** The MCP TypeScript SDK, the stdio/HTTP transports, the filter/object/dictionary stores — all of this is designed and prototyped in TypeScript. The existing `memory-engine.ts`, `sqlite-engine.ts`, and the whole config/loader system are Node. Rewriting in Python to get pandas native costs far more than it saves.

**Pandas belongs behind a sidecar, not inside the registry.** Treat it as Option B: a local HTTP service (or subprocess) that speaks the `QueryDefinition` JSON contract. The Node MCP server talks to it via an `HttpQueryEngine` adapter pointed at `localhost`. This way:

- The adapter registry stays purely Node/TypeScript
- The pandas logic can be written in idiomatic Python with no JS contamination
- The sidecar is independently replaceable (swap pandas for Polars, or a real remote service, with no MCP server changes)
- It degrades cleanly: if the pandas sidecar isn't running, the tool's `engine` config just points elsewhere

**IndexedDB is the one genuinely different case** — it only exists in browsers, so it's only relevant if there's ever a browser-embedded client-side MCP runtime. For now it's a future-planned adapter, not a v1 concern. Note it in the roadmap as "browser-only, requires a separate build target."

## What this means for the adapter registry spec

The registry becomes simpler, not more complex, once this is settled:
...
// All in-process adapters register here
const registry = new Map<string, AdapterFactory<unknown>>();

function registerAdapter<T>(name: string, factory: AdapterFactory<T>): void {
  registry.set(name, factory as AdapterFactory<unknown>);
}

// Built-in registrations at bootstrap
registerAdapter("memory",  MemoryAdapterFactory);
registerAdapter("sqlite",  SqliteAdapterFactory);   // wraps sqlite3/bun:sqlite
registerAdapter("pg",      PostgresAdapterFactory); // wraps pg driver
registerAdapter("http",    HttpAdapterFactory);     // covers pandas sidecar, remote APIs
registerAdapter("file",    FileAdapterFactory);     // covers json/jsonl/csv reads
```

`"pandas"` is never a registered name — it's just an `http` adapter whose URL points at a local sidecar. The registry doesn't need to know pandas exists.

One note on Bun specifically: Bun ships `bun:sqlite` natively, so `SqliteAdapterFactory` for a Bun-based MCP server doesn't need the `sqlite3` npm package at all — it's a built-in. Worth calling out in the roadmap since this was a Bun project from the start.

---

## Dictionary Table Schemas for Filter Engine Reuse

To implement `dictionary.find(query)` as a generic filter query over the dictionary's own data tables, the Dictionary Middleware exposes its core datasets under three standard `TableSchema` definitions. The filter query engine executes directly against these schemas:

### 1. `concepts` table
Represents the registered canonical concepts standard library.
- **Filterable Properties**:
  - `id`: `string` (UUID unique key)
  - `namespaceCode`: `string` (e.g. `'SNOMED'`, `'ICD-10'`, `'CUSTOM'`)
  - `standardCode`: `string` (e.g. `'38341003'`)
  - `display`: `string` (fallback description string)
- **Supported Operators**: `eq`, `neq`, `like`, `in_set`

### 2. `expressions` table (aliases)
Represents the shorthand alias expressions or text/regex matching patterns.
- **Filterable Properties**:
  - `id`: `string` (UUID)
  - `term`: `string` (human-readable alias)
  - `regexPattern`: `string` (regex match pattern)
  - `isCaseInsensitive`: `boolean`
  - `targetAssignment`: `string` (`'MAIN_TERM'` | `'ATTRIBUTE_MODIFIER'` | `'BOTH'`)
  - `conceptId`: `string` (foreign key to `concepts.id`)
  - `priorityWeight`: `number` (sorting priority)
  - `active`: `boolean`
  - `workspace_id`: `string` (isolation partition key, defaults to `'global'`)
  - `tags`: `array` (string categorization tags)
- **Supported Operators**: `eq`, `neq`, `like`, `in_set`, `not_in_set`

### 3. `relations` table
Tracks translation links and mappings between concepts.
- **Filterable Properties**:
  - `id`: `string`
  - `conceptId`: `string` (source concept ID)
  - `linkedId`: `string` (target concept ID)
  - `relationshipType`: `string` (`'EQUIVALENT'` | `'NARROWER_THAN'` | `'WIDER_THAN'`)
  - `active`: `boolean`
- **Supported Operators**: `eq`, `neq`

---

## MCP Server Entrypoint & Tool Schemas

All three middleware stores (Filter, Dictionary, and Object) register their public interfaces as standard MCP tools. 

### Context Injection Strategy
Since MCP transports do not natively carry state headers, **`session_id`** and **`userId`** are treated as request parameters in the tool's input schemas. The MCP server entrypoint extracts these arguments from the tool call payload and threads them directly to the store calls:
- All ephemeral session-scoped actions require `session_id`.
- Durable persistent operations (`*_save`) require `scope_level` ("user" or "global"). If "user", `userId` must be provided.

### 1. Filter Middleware Tools
- **`filter_init`**: Create a new filter version chain.
  - *Schema*: `{ tool_name?: string, table_name?: string, session_id: string }`
- **`filter_add`**: Append filter rules.
  - *Schema*: `{ filter_id: string, operations: Array<{ property: string, operator: string, value: any }>, session_id: string }`
- **`filter_combine`**: Combine multiple filters via set operations.
  - *Schema*: `{ operation: "union" | "intersection" | "difference" | "symmetric_difference", ids: string[], session_id: string }`
- **`filter_compress`**: Collapse version chain into a standalone root node.
  - *Schema*: `{ filter_id: string, session_id: string }`
- **`filter_get_filter`**: Fetch a filter version's details.
  - *Schema*: `{ filter_id: string, session_id: string }`
- **`filter_init_modifier`**: Initialize a GROUP BY/projection modifier.
  - *Schema*: `{ filter_id?: string, session_id: string }`
- **`filter_modifier_add`**: Add grouping columns and aggregations.
  - *Schema*: `{ mod_id: string, columns: string[], aggregations: Array<{ function: string, property: string, alias: string }>, session_id: string }`
- **`filter_init_view`**: Materialize view (combines filter, modifier, limit, offset).
  - *Schema*: `{ filter_id: string, mod_id?: string, having_id?: string, limit?: number, offset?: number, session_id: string }`
- **`filter_save_filter`**: Promote to persistent global/user storage.
  - *Schema*: `{ id: string, tags: string[], description: string, scope_level: "global" | "user", userId?: string, session_id: string }`
- **`filter_inspect`**: Retrieve detailed resolved rules and snapshotted tables.
  - *Schema*: `{ filter_id: string, session_id: string }`
- **`filter_diff`**: Get rules delta between two versions of the same chain.
  - *Schema*: `{ id_a: string, id_b: string, session_id: string }`

### 2. Dictionary Middleware Tools
- **`dictionary_add`**: Add an alias term mapping.
  - *Schema*: `{ term: string, canonical_concept: string, tags?: string[], description?: string, workspace_id?: string }`
- **`dictionary_find`**: Find registered alias expressions via structured filter.
  - *Schema*: `{ query?: string, tags?: string[], concept_type?: string, workspace_id?: string }`
- **`dictionary_resolve`**: Resolve a term to its single best canonical concept.
  - *Schema*: `{ term: string, workspace_id?: string }`
- **`dictionary_remove`**: Remove an alias expression mapping.
  - *Schema*: `{ dict_entry_id: string }`

### 3. Object Middleware Tools
- **`object_init`**: Create empty object instance against a schema definition.
  - *Schema*: `{ schema_name: string, session_id: string }`
- **`object_from_saved`**: Pre-populate a session object from a persistent template.
  - *Schema*: `{ saved_id: string, session_id: string }`
- **`object_set`**: Set a single nested property value.
  - *Schema*: `{ object_id: string, path: Array<string | number>, value: any, session_id: string }`
- **`object_patch`**: Overlay a partial delta object.
  - *Schema*: `{ object_id: string, partial: Record<string, any>, session_id: string }`
- **`object_ref`**: Embed a lazy reference to B's field on B from A.
  - *Schema*: `{ object_id: string, path: Array<string | number>, source_object_id: string, source_path: Array<string | number>, session_id: string }`
- **`object_array_append`**: Append an empty array element slot.
  - *Schema*: `{ object_id: string, path: Array<string | number>, session_id: string }`
- **`object_array_remove`**: Delete an element at a given index.
  - *Schema*: `{ object_id: string, path: Array<string | number>, index: number, session_id: string }`
- **`object_compress`**: Collapse object checkpoints to a single root state.
  - *Schema*: `{ object_id: string, session_id: string }`
- **`object_validate`**: Run schema completeness and validation checks.
  - *Schema*: `{ object_id: string, session_id: string }`
- **`object_save`**: Persist object to user/global template storage.
  - *Schema*: `{ object_id: string, tags: string[], description: string, scope_level: "global" | "user", userId?: string, session_id: string }`
- **`object_resolve`**: Materialize final JSON tool payload.
  - *Schema*: `{ object_id: string, mode: "tool_call" | "function", session_id: string }`
- **`object_inspect`**: Inspect detailed object tree and validation details.
  - *Schema*: `{ object_id: string, session_id: string }`
- **`object_diff`**: Get structural delta between two versions.
  - *Schema*: `{ id_a: string, id_b: string, session_id: string }`

---

## Error Taxonomy & Programmatic Error Responses

The MCP server responds to error scenarios by returning a standard payload with `isError: true` and a serialized JSON object inside the text content block. This structure allows client systems to parse error codes programmatically:
...
```json
{
  ...
  "content": [
    {
      "type": "text",
      "text": "{\n  \"error\": {\n    \"code\": \"SCHEMA_VALIDATION_ERROR\",\n    \"message\": \"Property 'price' is not filterable on table 'items'.\",\n    \"details\": {\n      \"property\": \"price\",\n      \"allowed_properties\": [\"name\", \"category\"]\n    }\n  }\n}"
    }
  ]
}
```
...
1. **`SCHEMA_VALIDATION_ERROR`**
   - *Cause*: A filter property/operator check failed against TableSchema, or an object value violates JSON Schema types.
2. **`INVALID_FILTER_ID` / `INVALID_OBJECT_ID`**
   - *Cause*: A referenced UUID state could not be resolved in the session, user, or global repositories.
3. **`MOCK_EXECUTION_ERROR`**
   - *Cause*: The filter failed to execute successfully against the local mock fixture database (e.g. malformed type coercion, aggregate without GROUP BY).
4. **`CYCLE_DETECTED`**
   - *Cause*: An object schema load detected a recursive type loop, or a lazy `object.ref` mapping created a cycle between live objects.
5. **`INVALID_PATH`**
   - *Cause*: Nested array index or property key path traversal failed during set, patch, or ref operations.
6. **`UNAUTHORIZED`**
   - *Cause*: A tool call attempted to write to `global` scope level without appropriate write privileges.
7. **`RESOURCE_NOT_FOUND`**
   - *Cause*: A configured schema file path, constant endpoint, or external API returned a 404/not found error.
