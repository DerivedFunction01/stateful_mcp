...
import { MemorySessionFilterStore, MemoryPersistentFilterStore, MemorySessionObjectStore, MemoryPersistentObjectStore, MemorySessionFormStore, MemoryPersistentFormStore } from "../src/adapters/storage/memory-repo";
...
describe("Cross-Service Referential Integrity", () => {
  ...
  test("Object validation fails when referenced filter ID does not exist", async () => {
    ...
    const objId = await objectStore.init("report_template", session, undefined, {
    ...
    expect(valResult.invalid?.[0]?.reason).toContain("does not point to a valid filter");
  });
  ...
  test("Form answer validation succeeds when referenced object ID exists", async () => {
    ...
    const result = await formStore.answer(formId, "q_patient_id", objId, session);
