...
describe("DataFrame Query Engine (Direct DuckDB)", () => {
  ...
  test("DataFrame write logging and compaction", async () => {
    ...
    await engine.appendRow({ item_id: 6, name: "syringe", category: "devices", price: 1.49, qty: 200 });
    ...
    const allRows = (await engine.execute("inventory", { filters: [] })) as any[];
    ...
    const postCompactRows = (await engine.execute("inventory", { filters: [] })) as any[];
