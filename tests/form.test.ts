...
import { MemorySessionFormStore, MemoryPersistentFormStore } from "../src/adapters/storage/memory-repo";
import { JsonlSessionFormStore, JsonlPersistentFormStore } from "../src/adapters/storage/jsonl-repo";
...
describe("Stateful Form Service", () => {
  ...
  describe.each([
    ["Memory Store", () => new FormStore(new MemorySessionFormStore(), new MemoryPersistentFormStore(), schemas)],
    ["JSONL Store", () => new FormStore(new JsonlSessionFormStore(JSONL_SESSION), new JsonlPersistentFormStore(JSONL_PERSISTENT), schemas)],
    ["SQLite Store", () => new FormStore(new SqliteFormStore(SQLITE_DB), new SqliteFormStore(SQLITE_DB), schemas)]
    ...
    test("Conditional branching on answers", async () => {
      ...
      let smokerRes = await store.answer(res.form_id, "q_smoke", true, session);
      ...
      let nonSmokerRes = await store.answer(res.form_id, "q_smoke", false, session);
    ...
    test("Range validations (scale)", async () => {
      ...
      expect(store.answer(res.form_id, "q_smoke_frequency", 0, session)).rejects.toThrow();
      expect(store.answer(res.form_id, "q_smoke_frequency", 150, session)).rejects.toThrow();
    ...
    test("Back-navigation and stale answer tracking", async () => {
      ...
      const changedRes = await store.answer(backRes.form_id, "q_smoke", false, session);
