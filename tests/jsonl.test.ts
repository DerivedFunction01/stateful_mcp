...
import { JsonlSessionFilterStore } from "../src/adapters/storage/jsonl-repo";
import { JsonlSessionObjectStore } from "../src/adapters/storage/jsonl-repo";
import { JsonlSessionEventStore } from "../src/adapters/storage/jsonl-repo";
...
describe("JSONL Persistent Storage Adapters", () => {
  ...
  test("JsonlSessionFilterStore - persistence, rehydration and prune", async () => {
    ...
    const filterId = await store.create("session_1", {
      parentFilterId: null,
      toolName: "test_tool",
    ...
    const filterId2 = await store.create("session_1", {
      parentFilterId: filterId,
      toolName: "test_tool",
      tableName: "test_table",
      rules: [{ property: "age", operator: "gt", value: 30 }, { property: "status", operator: "eq", value: "active" }],
      linearDepth: 1,
      gcLock: false,
      createdAt: new Date().toISOString()
    }, "active_users");
    ...
    const parsedEntries = finalRaw.trim().split("\n").map(l => JSON.parse(l));
    
    // Pruned state filterId, so only state filterId2 and its aliases survive
    const hasFilterIdState = parsedEntries.some(e => e.type === "state" && e.data.filterId === filterId);
    const hasFilterId2State = parsedEntries.some(e => e.type === "state" && e.data.filterId === filterId2);
  ...
  test("JsonlSessionObjectStore - persistence & rehydration", async () => {
    ...
    const objId = await store.create("session_2", {
      parentObjectId: null,
      schemaName: "profile",
      data: { username: "alice" },
      linearDepth: 0,
  ...
  test("JsonlSessionEventStore - persistence & rehydration", async () => {
    ...
    const commitId = await store.create("session_3", {
      parentCommitId: null,
      sessionId: "session_3",
      operation: "add",
      mutations: [{ event_id: "rec_1", type: "add", data: { name: "apple" } }],
      linearDepth: 0,
      gcLock: false,
      createdAt: new Date().toISOString()
    }, "tip");
