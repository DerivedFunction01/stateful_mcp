...
export class UniversalWordSegmenter {
	...
	isWordBoundary(text: string, start: number, end: number): boolean {
		...
		if (this.policy === "custom") {
			const beforeOk = start === 0 || (this.beforeRegex ? this.beforeRegex.test(text.slice(0, start)) : true);
			const afterOk = end === text.length || (this.afterRegex ? this.afterRegex.test(text.slice(end)) : true);
		...
		const isCjkPrev = /[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Hangul}]/u.test(prevChar);
		const isCjkNext = /[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Hangul}]/u.test(nextChar);
		const isCjkTarget = /[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Hangul}]/u.test(text.slice(start, end));
		...
		if (this.segmenter) {
			...
			const isStartBoundary = start === 0 || segments.some((s) => s.index === start);
			const isEndBoundary = end === text.length || segments.some((s) => s.index === end);
...
export class UniversalNumberParser {
	...
	normalize(text: string): { normalizedText: string; matches: Array<{ original: string; value: number }> } {
