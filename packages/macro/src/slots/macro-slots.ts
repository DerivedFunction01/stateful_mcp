...
import type { MacroLockLike, MacroSlotProjection, SlotBinding } from "../contracts/slots";
...
export function projectMacroSlots(
	...
	return parsed.matches.flatMap((match) => {
		const argument = spec.arguments.find((candidate) => candidate.argumentId === match.argumentId);
		if (!argument) return [];
		return [{
			macroId: spec.id,
...
export function activeMacroSlot(
	...
	return projections.find((slot) => cursorOffset >= slot.start && cursorOffset <= slot.end);
}
...
export function nextMacroSlot(
	...
	return direction > 0
		...
		: [...ordered].reverse().find((slot) => slot.end < cursorOffset) ?? ordered[ordered.length - 1];
}
...
): MacroLockLike & { lockedAtRevision: number; source: "explicit" | "accepted" } {
...
export function shiftMacroLocksForInsertion<T extends MacroLockLike>(
	...
	return locks.flatMap((lock) => {
		...
		return cursorOffset <= lock.start
			? [{ ...lock, start: lock.start + insertedLength, end: lock.end + insertedLength }]
			: [lock];
	});
}
...
export function shiftMacroLocksForDeletion<T extends MacroLockLike>(
	...
	return locks.flatMap((lock) => {
		...
		return lock.start >= deleteEnd
			? [{ ...lock, start: lock.start - (deleteEnd - deleteStart), end: lock.end - (deleteEnd - deleteStart) }]
			: [lock];
	});
}
...
export function removeMacroLock<T extends MacroLockLike>(
	...
	return locks.filter((lock) => lock.argumentId !== target.argumentId || lock.start !== target.start || lock.end !== target.end);
}

export function upsertMacroLock<T extends MacroLockLike>(locks: readonly T[], lock: T): T[] {
	const index = locks.findIndex((candidate) => candidate.macroId === lock.macroId && candidate.macroVersion === lock.macroVersion && candidate.start === lock.start && candidate.argumentId === lock.argumentId);
...
export function applyMacroLocks(
	...
	return projections.map((projection) => {
		const lock = locks.find((candidate) => candidate.macroId === projection.macroId && candidate.macroVersion === projection.macroVersion && candidate.argumentId === projection.argumentId && candidate.start >= projection.start && candidate.end <= projection.end);
		if (!lock) return projection.argumentId === activeArgumentId ? { ...projection, status: "bound" } : projection;
		return { ...projection, start: lock.start, end: lock.end, rawText: lock.rawText ?? projection.rawText, displayText: lock.binding?.displayValue ?? projection.displayText, binding: lock.binding, status: "locked" };
	});
}

function projectionToLock(projection: MacroSlotProjection): MacroLockLike {
	return { argumentId: projection.argumentId, macroId: projection.macroId, macroVersion: projection.macroVersion, start: projection.start, end: projection.end, rawText: projection.rawText, binding: projection.binding };
}
