import type {
	WorkspacePersistenceParticipant,
...
export class WorkspaceSaveCoordinator {
	...
	getParticipants(): readonly WorkspacePersistenceParticipant[] {
		return [...this.participants.values()].sort((a, b) =>
	...
	async save(
		scope: "active" | "all",
		reason: WorkspaceSaveRequest["reason"] = "explicit",
	): Promise<SaveSummary> {
		const activeTabId = this.layout?.getSnapshot().activeTabId;
		const eligible = this.getParticipants().filter(
			(participant) =>
				(scope === "all" ||
					participant.scope === "workspace" ||
					participant.tabId === activeTabId) &&
		...
		for (const participant of eligible) {
			...
			if (result.status === "failed" || result.status === "needsConfirmation")
				break;
		}
		return {
			results,
			blocked: results.some(
				({ result }) =>
					result.status === "failed" || result.status === "needsConfirmation",
			),
		};
	}

	saveActive(reason: WorkspaceSaveRequest["reason"] = "explicit") {
		return this.save("active", reason);
	}
	saveAll(reason: WorkspaceSaveRequest["reason"] = "explicit") {
		return this.save("all", reason);
	}
	saveActiveAndClose() {
		return this.saveActive("close");
	}
	saveAllAndQuit() {
		return this.saveAll("quit");
	}
}
