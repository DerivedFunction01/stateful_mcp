...
describe("neutral payload compiler", () => {
	test("materializes nested paths from typed and backend values", () => {
		const result = compileMacroPayload(spec, "^note year=2004 title=hp", { backends: { books: backend }, mode: "execute" });
		expect(result.status).toBe("matched");
		expect(result.payload).toEqual({ foo: { path: { args: { title: { id: "book-series" }, year: 2004 } } } });
	});
	...
	test("keeps a prefix incomplete during live compilation", () => {
		...
		const result = compileMacroPayload(spec, "^note harry pot", { backends: { books: prefixBackend } });
		expect(result.status).toBe("incomplete");
		expect(result.arguments.find((argument) => argument.argumentId === "title")?.state).toBe("pending");
	});

	test("reports conflicting writes instead of overwriting", () => {
		const conflictSpec: MacroSpec = {
			...spec,
			arguments: spec.arguments.map((argument) => ({ ...argument, path: "same.value" })),
		};
		const result = compileMacroPayload(conflictSpec, "^note year=2004 title=hp", { backends: { books: backend }, mode: "execute" });
		expect(result.diagnostics.some((diagnostic) => diagnostic.code === "PATH_CONFLICT")).toBe(true);
	});
});
