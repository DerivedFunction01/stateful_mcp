...
const books = createExpressionBackendFixture([
	...
	{ id: "hp-series", term: "harry potter", canonicalValue: "series", priority: 1 },
	{ id: "hp-book", term: "harry potter and the deathly hallows", canonicalValue: "deathly-hallows", priority: 10 },
	{ id: "deathly", term: "deathly hallows", canonicalValue: "deathly-hallows", priority: 2 },
	{ id: "inactive", term: "disabled", canonicalValue: "disabled", active: false },
]);
...
describe("neutral macro parser", () => {
	test("recognizes the envelope and preserves named assignment spans", () => {
		const result = parseMacroLine('  ^note title="harry potter" year=2004', spec, { backends: { books } });
		expect(result?.macroName).toBe("note");
		expect(result?.arguments.map((argument) => argument.name)).toEqual(["title", "year"]);
	...
	test("matches unordered expression and numeric arguments", () => {
		const result = parseMacroLine("^note 2004 hp", spec, { backends: { books } });
		expect(result?.matches.map((match) => match.argumentId)).toEqual(["year", "title"]);
		expect(result?.matches.find((match) => match.argumentId === "title")?.canonicalValue).toBe("series");
	});

	test("keeps a longer expression as the precedence winner", () => {
		const result = parseMacroLine("^note harry potter and the deathly hallows", spec, { backends: { books } });
		expect(result?.matches.filter((match) => match.argumentId === "title")).toHaveLength(1);
		expect(result?.matches[0]?.canonicalValue).toBe("deathly-hallows");
	});

	test("reports a prefix expression for live pending state", () => {
		const result = parseMacroLine("^note harry pot", spec, { backends: { books } });
		expect(result?.matches[0]?.matchKind).toBe("prefix");
	});

	test("reports unknown named arguments and ignores inactive expressions", () => {
		const result = parseMacroLine("^note disabled bogus=value", spec, { backends: { books } });
		expect(result?.diagnostics.some((diagnostic) => diagnostic.code === "UNKNOWN_ARGUMENT")).toBe(true);
		expect(result?.matches.some((match) => match.rawValue === "disabled")).toBe(false);
	});
