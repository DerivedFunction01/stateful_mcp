...
function HostStory() {
	...
	return <Card title="Host-connected diagnostics" action={<Badge tone={error ? "danger" : snapshot ? "success" : "info"}>{error ? "Disconnected" : snapshot ? "Connected" : "Loading"}</Badge>}><div className="host-story"><div className="host-meta"><div><span className="field-label">Workspace</span><strong>{snapshot?.workspaceId ?? "—"}</strong></div><div><span className="field-label">Session</span><strong>{snapshot?.sessionId ?? "—"}</strong></div><div><span className="field-label">Profile</span><strong>{snapshot?.profileId ?? "—"}</strong></div></div>{snapshot && <><div className="extension-list"><span className="field-label">Enabled domain applications</span>{snapshot.enabledExtensionIds.map((id) => <div className="extension-row" key={id}><span><CheckCircle2 size={15} />{id}</span><Badge tone="success">Active</Badge></div>)}</div>{snapshot.diagnostics.map((item) => <Diagnostic key={item.message} severity={item.severity}>{item.message}</Diagnostic>)}</>}{error && <Diagnostic severity="error">{error}</Diagnostic>}<p className="story-note"><Code2 size={15} />This story uses the same snapshot shape as the Bun host transport.</p></div></Card>;
}
