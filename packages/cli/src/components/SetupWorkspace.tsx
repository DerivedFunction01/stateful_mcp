...
type Field =
	...
	| (typeof BLOCK_FIELDS)[number]
	| (typeof MACRO_FIELDS)[number];
...
export function SetupWorkspace({
	...
	const [fundamentalsPane, setFundamentalsPane] = useState<
		"date" | "numeric" | "measurement"
	>("date");
	...
	const setFormValue = (value: string) => {
		if (stage === "document")
			setPlacement((form) => ({ ...form, [currentField!]: value }));
		if (stage === "blocks")
			setBlock((form) => ({ ...form, [currentField!]: value }));
		if (stage === "macros")
			setMacro((form) => ({ ...form, [currentField!]: value }));
		setDraft("");
	};
		...
		update({
			placements: [
				...source.placements.filter(
					(item) => item.placementId !== next.placementId,
				),
				next,
			],
		});
	...
	const addBlock = (data = block) => {
		...
		const concept = {
			...
			namespaceCode: "LOCAL",
			standardCode: data["concept id"],
			display: data["concept display"] || data.phrase,
		};
		const expressionId = `expr-${data["concept id"]}`;
		const blockId = `block-${data["concept id"]}`;
		update({
			concepts: [
				...source.concepts.filter(
					(item) => item.conceptId !== concept.conceptId,
				),
				concept,
			],
			expressions: [
				...source.expressions.filter((item) => item.id !== expressionId),
				{
					id: expressionId,
					term: data.phrase,
					lookupTerm: data.phrase,
					regexPattern: data.phrase,
			...
			conceptFilters: [
				...source.conceptFilters.filter(
					(item) => item.filterId !== `filter-${blockId}`,
				),
				{
					filterId: `filter-${blockId}`,
					conceptId: concept.conceptId,
					roleName: `${data["target schema"]}.${data["target path"]}`,
					policy: "whitelist",
					active: true,
				},
			],
			blocks: [
				...source.blocks.filter((item) => item.blockId !== blockId),
				{
					blockId,
					version: 1,
					label: data["concept display"] || data.phrase,
					kind: "concept",
					target: {
						targetSchema: data["target schema"],
					...
					valueKind: "concept",
					source: { kind: "concept", conceptId: concept.conceptId },
					filterIds: [`filter-${blockId}`],
					schemaVersion: 1,
					status: "draft",
				},
			],
	...
	const addMacro = (data = macro) => {
		...
		const dateMode =
			data["date child"] === "shared"
				? "shared"
				: data["date child"] === "custom"
					? "custom"
					: "none";
		const next: SetupMacroComposition = {
			macroId: data["macro id"],
			version: 1,
			macroName: data["macro name"] || data["macro id"],
			targetSchema: data["target schema"],
			targetSchemaVersion: 1,
			allowedPlacementIds: source.placements
				.filter((item) => item.targetSchema === data["target schema"])
				.map((item) => item.placementId),
			parameters: source.blocks
				.filter((item) => item.target.targetSchema === data["target schema"])
				.map((item) => ({ argumentId: item.blockId, blockId: item.blockId })),
			dateChild: {
				mode: dateMode,
				...(dateMode === "custom"
					? { childMacroId: "simple-date", targetPath: "dateRange" }
					: dateMode === "shared"
		...
		update({
			macros: [
				...source.macros.filter((item) => item.macroId !== next.macroId),
				next,
			],
		});
		setMacro(emptyMacro());
	...
	const advance = (direction: number) => {
		setStage(
			STAGES[
				Math.max(0, Math.min(STAGES.length - 1, currentIndex + direction))
			]!,
		);
		setFieldIndex(0);
		setDraft("");
	...
	useInput((input, key) => {
		if (stage === "fundamentals") {
			...
			if (key.leftArrow || key.rightArrow)
				return advance(key.leftArrow ? -1 : 1);
			return;
		}
		if (key.escape) return onExit();
	...
	const form =
		stage === "document" ? placement : stage === "blocks" ? block : macro;
	return (
		<Box flexDirection="column" padding={1}>
			<Box justifyContent="space-between">
				<Text bold>Clinical setup</Text>
				<Text dimColor>Esc exit Tab field ←/→ stage Enter apply</Text>
			</Box>
			<Box marginTop={1}>
				{STAGES.map((item, index) => (
					<Text key={item} color={item === stage ? "cyan" : undefined}>
						{index < currentIndex ? "✓" : index === currentIndex ? "→" : "○"}{" "}
						{item} {index < STAGES.length - 1 ? "  " : ""}
					</Text>
				))}
			</Box>
			{stage === "fundamentals" && fundamentalsPane === "date" && (
				<DateTimeFormatWizard
					key={wizardVersion}
					...
					onConfirm={(format, examples, preferred) => {
						const formats = [
							...(source.primitiveProfile.dateTimeFormats ?? []).filter(
						...
						onChange({
							...
							primitiveProfile: {
								...
								...(preferred || !source.primitiveProfile.preferredDateFormat
									? { preferredDateFormat: format.id }
									: {}),
							},
			...
			{stage === "document" && (
				<EditorStage
					title="Document placements"
					fields={fields}
					fieldIndex={fieldIndex}
					draft={draft}
					form={form}
					hint="Add a reusable schema placement. Use one or many cardinality."
					items={source.placements.map(
						(item) =>
							`${item.placementId}: ${item.documentPath} → ${item.targetSchema} (${item.cardinality})`,
					)}
				/>
			)}
			{stage === "blocks" && (
				<EditorStage
					title="Concepts and grammar blocks"
					fields={fields}
					fieldIndex={fieldIndex}
					draft={draft}
					form={form}
					hint="One guided entry creates a concept, expression, and executable concept block."
					items={source.blocks.map(
						(item) =>
							`${item.label}: ${item.target.targetSchema}.${item.target.targetPath}`,
					)}
				/>
			)}
			{stage === "macros" && (
				<EditorStage
					title="Macro composer"
					fields={fields}
					fieldIndex={fieldIndex}
					draft={draft}
					form={form}
					hint="Macros include matching blocks for the selected target schema and an explicit date-child policy."
					items={source.macros.map(
						(item) =>
							`${item.macroName}: ${item.parameters.length} fields, ${item.allowedPlacementIds.length} placements, date=${item.dateChild?.mode ?? "none"}`,
					)}
				/>
			)}
			{stage === "review" && (
				<Box flexDirection="column" marginTop={1}>
					<Text bold>Semantic review</Text>
					<Text>
						Concepts {source.concepts.length} Expressions{" "}
						{source.expressions.length} Placements {source.placements.length}{" "}
						Blocks {source.blocks.length} Macros {source.macros.length}
					</Text>
					{result.diagnostics.length === 0 ? (
						<Text color="green">Ready to save draft</Text>
					) : (
					...
					<Text dimColor>
						Enter saves the source. Publish and activate it through the setup
...
function EditorStage({
	...
	return (
		<Box flexDirection="column" marginTop={1}>
			...
			<Text color="cyan">
				{fields[fieldIndex]}:{" "}
				{draft || form[fields[fieldIndex]!] || "type a value"}
			</Text>
			<Text dimColor>Tab moves between fields; Enter applies each value.</Text>
			{items.length === 0 ? (
				<Text color="yellow">No entries configured</Text>
			) : (
