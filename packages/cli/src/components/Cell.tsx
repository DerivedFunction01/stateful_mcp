...
export function CellComponent({
	...
	const displayText =
		isActive && mode === "INSERT"
			? (draftText ?? cell.authored.rawText)
			...
			>
				{isActive && mode === "MACRO" && draftText !== undefined ? (
					<MacroDraftText
						text={displayText}
						slots={macroSlots ?? []}
						activeArgumentId={activeMacroArgumentId}
				...
				{suggestions?.slice(0, 5).map((suggestion, suggestionIndex) => (
					<Text key={suggestionIndex} dimColor>
						▸ {suggestion.text}
			...
			{segments.map((segment, index) => {
				...
				if (segment.kind === "slot") {
					const slot = slots.find(
						(candidate) =>
							candidate.start <= text.indexOf(segment.text) &&
							candidate.end >= text.indexOf(segment.text) + segment.text.length,
					);
					return (
						<Text
							key={index}
							...
							color={
								slot?.status === "locked"
									? "magenta"
									: slot?.argumentId === activeArgumentId
										? "yellow"
										: "cyan"
							}
							bold
