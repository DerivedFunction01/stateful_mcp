...
describe("macro autocomplete state transitions", () => {
	test("Tab cycling cycles candidates without mutating draft in reduceCompletion", () => {
		...
		let state = reduceCompletion({ status: "idle" }, { kind: "tab", shift: false }, "^vi", provider, mockSyntaxProfile);
		...
		state = reduceCompletion(state.completionState, { kind: "tab", shift: false }, "^vi", provider, mockSyntaxProfile);
		expect(state.completionState.status).toBe("cycling");
		if (state.completionState.status === "cycling") {
		...
		state = reduceCompletion(state.completionState, { kind: "tab", shift: true }, "^vi", provider, mockSyntaxProfile);
		expect(state.completionState.status).toBe("cycling");
		if (state.completionState.status === "cycling") {
			expect(state.completionState.highlightIndex).toBe(0);
		}
	...
	test("Enter accepts the highlighted candidate and returns executeLine", () => {
		const provider = () => suggestions;
		const transition = reduceCompletion({ status: "idle" }, { kind: "tab", shift: false }, "^vi", provider, mockSyntaxProfile);
		
		// Enter on cycling state
		const enterTransition = reduceCompletion(transition.completionState, { kind: "enter" }, "^vi", provider, mockSyntaxProfile);
