...
import type { PipelineStep, OpName } from "../../translation/types";
...
export class MemoryVariableStore implements VariableStore {
  ...
  private getScopeMap(sessionId: string, blockInstanceId?: string): Map<string, unknown> {
  ...
  async save(sessionId: string, key: string, value: unknown, blockInstanceId?: string): Promise<void> {
    const map = this.getScopeMap(sessionId, blockInstanceId);
    map.set(key, value);
  }

  async saveBatch(sessionId: string, variables: Record<string, unknown>, blockInstanceId?: string): Promise<void> {
    const map = this.getScopeMap(sessionId, blockInstanceId);
  ...
  async load(sessionId: string, key: string, blockInstanceId?: string): Promise<unknown> {
    const map = this.getScopeMap(sessionId, blockInstanceId);
    return map.get(key);
  }

  async loadScope(sessionId: string, blockInstanceId?: string): Promise<Record<string, unknown>> {
    const map = this.getScopeMap(sessionId, blockInstanceId);
    return Object.fromEntries(map.entries());
  }

  async remove(sessionId: string, key: string, blockInstanceId?: string): Promise<void> {
    const map = this.getScopeMap(sessionId, blockInstanceId);
    map.delete(key);
  }
...
export class VariableServiceStore implements VariableService {
  ...
  private listeners = new Map<string, Set<(event: VariableMutationEvent) => void>>();
  ...
  async setVariable(sessionId: string, key: string, value: unknown, blockInstanceId?: string): Promise<void> {
  ...
  async setVariables(
    sessionId: string,
    variables: Record<string, unknown> | Array<{ key: string; value: unknown; blockInstanceId?: string }>,
    ...
    if (Array.isArray(variables)) {
      for (const entry of variables) {
        ...
        const storedValue = entry.value !== undefined
          ? entry.value
  ...
  async getVariable<T = unknown>(
    ...
    if (keyOrKeys === undefined) {
      return (await this.getScope(sessionId, blockInstanceId)) as Record<string, T>;
    }
    ...
    if (blockInstanceId) {
      const blockVal = await this.store.load(sessionId, keyOrKeys, blockInstanceId);
  ...
  async getScope(sessionId: string, blockInstanceId?: string): Promise<Record<string, unknown>> {
  ...
  async deleteVariable(sessionId: string, keyOrKeys: string | string[], blockInstanceId?: string): Promise<void> {
  ...
  async clearBlockScope(sessionId: string, blockInstanceId: string): Promise<void> {
  ...
  async testVariableCondition(
    ...
    let targetVals: unknown[] | undefined = undefined;
    let storedCondition: VariableConditionRule | undefined = undefined;

    if (rawVal !== undefined && typeof rawVal === "object" && rawVal !== null) {
      const obj = rawVal as Record<string, any>;
      if (obj.condition && typeof obj.condition === "object" && obj.condition.op) {
      ...
      } else if ((obj.targetValue !== undefined || obj.targetValues !== undefined) && obj.op) {
        storedCondition = { op: obj.op, targetValue: obj.targetValue, targetValues: obj.targetValues };
    ...
    const testArgs = isSetOp
      ? [testValue, ...(targetVals ?? (targetVal !== undefined ? [targetVal] : []))]
      : [testValue, targetVal];
    ...
    const conditionRule: VariableConditionRule = storedCondition || (
      isSetOp
  ...
  async evaluatePipeline(sessionId: string, pipeline: PipelineStep[], blockInstanceId?: string): Promise<unknown> {
