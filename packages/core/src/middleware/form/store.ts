...
export function resolveNavigationState(
	...
	while (currentQId) {
		...
		if (qDef?.next) {
			if (Array.isArray(qDef.next)) {
				...
				if (
					ans !== undefined &&
					(qDef.next as any)[String(ans)] !== undefined
				) {
	...
	let nextQuestions = activePath.filter(
		(qId) => answers[qId] === undefined && !skipped.includes(qId),
	);
	...
	const stale = allQuestions.filter(
		(qId) =>
			!visibleQuestions.has(qId) &&
			(answers[qId] !== undefined || skipped.includes(qId)),
	);
	...
	const revealed = activePath.filter(
		(qId) => answers[qId] === undefined && !skipped.includes(qId),
	);
	...
	switch (qDef.answer_type) {
		case "boolean":
			if (typeof value !== "boolean") {
				throw new Error(
					`Invalid answer: Expected boolean, received ${typeof value}`,
				);
			}
			break;
		case "number":
		...
		case "multiple_choice":
			if (!qDef.options?.includes(value)) {
				throw new Error(
					`Invalid choice "${value}". Valid options: ${qDef.options?.join(", ")}`,
				);
			}
			break;
		case "multi_select":
		...
		case "scale":
			...
			if (qDef.scale) {
				if (value < qDef.scale.min || value > qDef.scale.max) {
					throw new Error(
						`Value ${value} out of range [${qDef.scale.min}, ${qDef.scale.max}]`,
					);
				}
			}
			break;
		case "ranked":
			if (!Array.isArray(value)) {
				throw new Error(
					"Invalid answer: Expected an array for ranked options.",
				);
			}
			break;
		case "free_text":
...
export class FormStore {
	...
	public async answer(
		...
		if (!qDef) {
			throw new Error(
				`Question "${questionId}" does not exist in schema "${parentState.schemaName}".`,
			);
		}
		...
		if ((qDef as any)["x-mcp-ref"]) {
			...
			if (refType === "filter" && this.filterStore) {
				...
				if (!exists) {
					throw new Error(
						`Invalid reference: Answer "${idOrAlias}" is not a valid filter ID in session.`,
					);
				}
				...
				if (!exists) {
					throw new Error(
						`Invalid reference: Answer "${idOrAlias}" is not a valid object ID in session.`,
					);
				}
			}
		}
		...
		const newFormId = await this.sessionStore.create(
			sessionId,
			newState,
			newAlias,
		);
	...
	public async resolve(formIdOrAlias: string, sessionId: string): Promise<any> {
		...
		if (!nav.complete) {
			const unansweredRequired = Object.entries(schema.questions)
				.filter(
					([qId, q]) =>
						q.required &&
						state.answers[qId] === undefined &&
						!state.skipped.includes(qId),
				)
				.map(([qId]) => qId);
			throw new Error(
				`Form is not complete. Unanswered required questions: ${unansweredRequired.join(", ")}`,
			);
		}
		...
		const activeSkipped = state.skipped.filter((qId: string) =>
			nav.visibleQuestions.includes(qId),
		);
	...
	public async compress(formId: string, sessionId: string): Promise<string> {
		...
		const activeSkipped = form.skipped.filter((qId: string) =>
			nav.visibleQuestions.includes(qId),
		);
