...
export class EventStore {
  ...
  async getCommit(commitId: string, sessionId: string): Promise<EventCommit | null> {
  ...
  async delete(sessionId: string, idOrAlias: string, eventId: string, alias?: string): Promise<string> {
    const resolvedId = await this.resolveId(idOrAlias, sessionId);
    const parent = await this.session.get(sessionId, resolvedId);
    if (!parent) {
      throw new StatefulFrameworkError(ErrorCode.OBJECT_NOT_FOUND, `Parent commit "${idOrAlias}" not found`);
  ...
  private async getSchemaName(commitId: string, sessionId: string): Promise<string> {
  ...
  private async findLastMutationCommitId(startCommitId: string, eventId: string, sessionId: string): Promise<string | null> {
  ...
  async merge(
  ...
  ): Promise<{ status: "clean" | "conflict"; commit_id?: string; merge_session_id?: string; conflicts?: MergeConflict[] }> {
    const resolvedTarget = await this.resolveId(targetIdOrAlias, sessionId);
    const resolvedSources = await Promise.all(sourceIdsOrAliases.map((s) => this.resolveId(s, sessionId)));
    ...
    const lcaEventsMap = new Map<string, EventRecord>(stateLca.map((e) => [e.event_id, e]));
    ...
    if (multiMutatedEventIds.size > 0) {
      ...
      for (const eventId of multiMutatedEventIds) {
        ...
        const targetValue = targetMut ? (targetMut.type === "remove" ? null : targetMut.data) : lcaEventsMap.get(eventId);
      ...
      return { status: "conflict", merge_session_id: mergeSessionId, conflicts };
    }
    ...
    const isAliasInput = (await this.session.getAlias(sessionId, targetIdOrAlias)) !== null;
  ...
  async mergeCommit(mergeSessionId: string, sessionId: string): Promise<string> {
    ...
    const lca = await this.findLCA([session.targetCommitId, ...session.sourceCommitIds], sessionId);
    ...
    for (const conflict of session.conflicts) {
      ...
      if (res.strategy === "accept_source") {
        const resolvedSourceId = await this.resolveId(res.source_id!, sessionId);
        const sourceVal = conflict.source_values[resolvedSourceId];
        if (sourceVal === null) {
          resolvedMutations.push({ type: "remove", event_id: conflict.event_id });
        } else {
          resolvedMutations.push({ type: "update", event_id: conflict.event_id, data: sourceVal });
        }
        ...
        if (conflict.target_value === null) {
          ...
          resolvedMutations.push({ type: "update", event_id: conflict.event_id, data: conflict.target_value });
        }
        acceptedIds.push(session.targetCommitId);
      } else if (res.strategy === "patch") {
        resolvedMutations.push({ type: "update", event_id: conflict.event_id, data: res.values });
      }
    }
    ...
    const parentCommit = await this.session.get(sessionId, session.targetCommitId);
    const linearDepth = ((parentCommit?.linearDepth || 1) + 1);
    ...
    const isAliasInput = (await this.session.getAlias(sessionId, session.targetCommitId)) !== null;
  ...
  private async compressSuffix(commitId: string, sessionId: string): Promise<string> {
    const tipNode = await this.session.get(sessionId, commitId);
    if (!tipNode) return commitId;

    // Find the nearest branch point
    const { branchPointId, commitsToCompress } = await this.findNearestBranchPoint(commitId, sessionId);
    if (commitsToCompress.length <= 1) {
      return commitId;
    }

    // Project state at branch point and tip
    const stateLca = branchPointId ? await this.project(branchPointId, sessionId) : [];
    const stateTip = await this.project(commitId, sessionId);

    // Diff states
    const lcaMap = new Map<string, EventRecord>(stateLca.map((e) => [e.event_id, e]));
    const tipMap = new Map<string, EventRecord>(stateTip.map((e) => [e.event_id, e]));
    ...
    for (const [eventId, event] of tipMap.entries()) {
      ...
      if (!original) {
        ...
        for (const [key, val] of Object.entries(event)) {
          if (key !== "event_id" && JSON.stringify(val) !== JSON.stringify(original[key])) {
            patch[key] = val;
            modified = true;
          }
        }
  ...
  private async findNearestBranchPoint(
    commitId: string,
    sessionId: string
  ): Promise<{ branchPointId: string | null; commitsToCompress: EventCommit[] }> {
    ...
    while (currentId) {
      ...
      if (node.parentCommitId) {
        const siblings = await this.session.listChildren(sessionId, node.parentCommitId);
  ...
  async gc(
    ...
    const keptAliases = aliases.filter((a) => {
      ...
      if (whitelistAliases.length > 0) return whitelistAliases.includes(a.alias);
      return true;
    });
