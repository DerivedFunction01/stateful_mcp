...
import type { OwnerScope, TableSchema } from "../../config/types";
...
export class FilterStore {
	...
	private async lookup(
		...
		const fromPersistent =
			(userId
				? await this.persistent.get(resolvedId, { level: "user", userId })
				: null) ?? (await this.persistent.get(resolvedId, { level: "global" }));
	...
	private getPropertyType(
		...
		if (schema.mock_dataset && schema.mock_dataset.length > 0) {
			...
			if (
				firstRow &&
				firstRow[property] !== undefined &&
				firstRow[property] !== null
			) {
	...
	async init(
		...
		if (toolName) {
			...
			if (tableName && !tables[tableName]) {
				throw new StatefulFrameworkError(
					...
					`Table "${tableName}" not in tool "${toolName}"`,
				);
			}
		}
		...
		const schema =
			toolName && tableName
				? this.toolSchemas.get(toolName)?.[tableName]
				: undefined;
	...
	async add(
		...
		const isAliasInput =
			(await this.session.getAlias(sessionId, filterId)) !== null;
		const targetAlias = newAlias || (isAliasInput ? filterId : undefined);

		const childId = await this.session.create(
			sessionId,
			childState,
			targetAlias,
		);
	...
	private async findNearestBranchPoint(
		...
		rulesToCompress: FilterCondition[];
	}> {
	...
	private async compressSuffix(
		...
		const { branchPointId, rulesToCompress } =
			await this.findNearestBranchPoint(resolvedId, sessionId, userId);
	...
	async diff(
		...
		if (!filterA || !filterB) {
			throw new StatefulFrameworkError(
				ErrorCode.FILTER_NOT_FOUND,
				"One or both filters for diff not found",
			);
		}

		if (
			filterA.toolName !== filterB.toolName ||
			filterA.tableName !== filterB.tableName
		) {
			throw new StatefulFrameworkError(
		...
		const serializeRule = (r: FilterCondition) =>
			JSON.stringify({
				property: r.property,
				operator: r.operator,
				value: r.value,
	...
	async compress(
		...
		if (filter.combined_operation) {
			throw new StatefulFrameworkError(
				...
				"Combined filters cannot be compressed",
			);
		}
	...
	async combine(
		...
		if (ids.length < 2) {
			throw new StatefulFrameworkError(
				ErrorCode.FILTER_COMBINATION_INVALID,
				"Need at least 2 filter IDs to combine",
			);
		}
	...
	async save(
		...
		if (scope.level === "global") {
			if (!checkPrivilege(undefined)) {
				throw new StatefulFrameworkError(
					ErrorCode.FILTER_PRIVILEGE_DENIED,
					"Insufficient privilege for global scope",
				);
			}
		}
		...
		if (!state) {
			throw new StatefulFrameworkError(
				ErrorCode.FILTER_NOT_FOUND,
				`Filter "${filterId}" not in session`,
			);
		}
	...
	async resolveRows(
		...
		if (!filter.toolName || !filter.tableName) {
			throw new StatefulFrameworkError(
				ErrorCode.FILTER_COMBINATION_INVALID,
				"Filter not bound to a tool/table",
			);
		}
		...
		if (!queryEngineResolver) {
			throw new StatefulFrameworkError(
				...
				"QueryEngine resolver not provided",
			);
		}
	...
	public async removeRule(
		...
		const resolvedRules = await this.getFilterRules(
			filterId,
			sessionId,
			userId,
		);
