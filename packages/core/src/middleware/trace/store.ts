...
export class TraceStore {
	...
	public stopRecording(
		...
		if (!session) {
			throw new Error(
				`No active recording session found for trace_id "${traceId}".`,
			);
		}
	...
	public recordTrace(rawForm: TraceForm): TraceForm {
		...
		const inferredInputSlots: Record<string, TraceSlot> = {
			...rawForm.input_slots,
		};
		...
		for (const step of rawForm.steps) {
			...
			const hasExplicitSlots =
				Object.keys(rawForm.input_slots || {}).length > 0;
				...
				for (const [key, val] of Object.entries(step.args)) {
					...
					if (shouldForce) {
						if (val !== undefined && val !== null) {
							const slotKey = inferredInputSlots[key]
								? `${step.action}_${key}`
								: key;
						...
						for (const [slotKey, slotDef] of Object.entries(
							rawForm.input_slots || {},
						)) {
							if (slotDef.target) {
								const t = slotDef.target;
								const stepMatch = t.step_id
									? step.id === t.step_id || autoId === t.step_id
									: true;
								const actionMatch = t.action ? step.action === t.action : true;
								const occMatch = t.occurrence
									? actionCounts[baseName] === t.occurrence
									: true;
								...
								(val === slotDef.default ||
									key === slotKey ||
									key.toLowerCase().includes(slotKey.toLowerCase()))
							) {
								parameterizedArgs[key] = `$input.${slotKey}`;
								break;
							}
		...
		const traceId =
			rawForm.trace_id || `trc_${Math.random().toString(36).slice(2, 10)}`;
	...
	public queryTraces(intent: string, limit = 10, offset = 0): TraceQueryResult {
		...
		for (const trace of this.traces.values()) {
			...
			if (trace.goal.toLowerCase().includes(lowerIntent)) {
				...
				const hitCount = words.filter((w) =>
					trace.goal.toLowerCase().includes(w),
				).length;
				if (hitCount > 0) {
			...
			if (score > 0) {
				matches.push({
					...
					confidence_score: Number(
						(trace.confidence_score! * score).toFixed(2),
					),
	...
	public async executeTrace(
		...
		for (const [slotKey, slotDef] of Object.entries(trace.input_slots || {})) {
			if (
				slotDef.required &&
				(inputArgs[slotKey] === undefined || inputArgs[slotKey] === null)
			) {
		...
		let currentStepId: string | undefined =
			trace.start_step || trace.steps[0]?.id;
		...
		while (currentStepId) {
			...
			if (limits.max_total && currentActionCount > limits.max_total) {
				return this.handleFailure(
					traceId,
					`Exceeded max_total execution limit for tool "${step.action}".`,
					rollbackStack,
					toolExecutor,
				);
			}
			...
			if (
				limits.max_consecutive &&
				consecutiveCounts.count > limits.max_consecutive
			) {
				return this.handleFailure(
					traceId,
					`Exceeded max_consecutive execution limit for tool "${step.action}".`,
					rollbackStack,
					toolExecutor,
				);
			}
			...
			// Resolve arguments
			const resolvedArgs = this.resolveArgs(
				step.args || {},
				inputArgs,
				stepResults,
			);
			...
			try {
				...
				if (
					step.error_targets &&
					err.message &&
					step.error_targets[err.message]
				) {
				...
				return this.handleFailure(
					traceId,
					err.message || "Step execution failed.",
					rollbackStack,
					toolExecutor,
				);
			}
			...
			if (step.transactional?.rollback_action) {
				const rollbackArgs = this.resolveArgs(
					step.transactional.rollback_args || {},
					inputArgs,
					stepResults,
					stepOutput,
				);
				rollbackStack.push({
					action: step.transactional.rollback_action,
					args: rollbackArgs,
				});
			}
	...
	public async resumeTrace(
		...
		if (!state) {
			return {
				status: "failed",
				trace_id: "unknown",
				error: `Invalid or expired resume token "${resumeToken}".`,
			};
		}
		...
		const currentStepIndex = trace.steps.findIndex(
			(s) => s.id === state.step_id,
		);
		...
		return this.executeTraceFromStep(
			trace,
			nextStepId,
			state.input_args,
			state.step_results,
	...
	public refineTrace(traceId: string, delta: DeltaOperation): TraceForm {
		...
		switch (delta.action) {
			case "replace_step": {
				if (!delta.step_id || !delta.new_step)
					throw new Error("replace_step requires step_id and new_step.");
			...
			case "append_step": {
				...
				if (delta.target_step_id) {
					...
					if (idx === -1)
						throw new Error(`Target step "${delta.target_step_id}" not found.`);
			...
			case "swap_with_persistent": {
				if (!delta.step_id || !delta.persistent_key) {
					throw new Error(
						"swap_with_persistent requires step_id and persistent_key.",
					);
				}
				const idx = steps.findIndex((s) => s.id === delta.step_id);
				if (idx === -1) throw new Error(`Step "${delta.step_id}" not found.`);
			...
			case "promote_arg": {
				...
				const slotDef: TraceSlot = delta.slot_def || {
					...
					required: false,
					default:
						typeof currentVal === "string" && currentVal.startsWith("$input.")
							? undefined
							: currentVal,
				};
			...
			case "demote_arg": {
				...
				const fallbackVal =
					delta.literal_value !== undefined
						? delta.literal_value
						: slotName && trace.input_slots?.[slotName]?.default !== undefined
							? trace.input_slots[slotName]!.default
							: currentVal;

				const updatedArgs = {
					...(step.args || {}),
					[delta.arg_key]: fallbackVal,
				};
				steps[idx] = { ...step, args: updatedArgs };
				...
				if (slotName) {
					const isStillReferenced = steps.some((s) =>
						Object.values(s.args || {}).some(
							(v) => typeof v === "string" && v === `$input.${slotName}`,
						),
					);
		...
		for (const [key, val] of Object.entries(template)) {
			if (typeof val === "string") {
				if (val.startsWith("$input.")) {
					...
					let target =
						(stepId ? stepResults[stepId] : undefined) ||
						(stepId === "current" ? currentStepOutput : undefined);
	...
	private async handleFailure(
		...
		while (rollbackStack.length > 0) {
			...
			try {
				...
				rollbackLog.push(
					`Rolled back ${item.action} with ${JSON.stringify(item.args)}`,
				);
			} catch (err: any) {
