...
function applyOp(step: PipelineStep, args: unknown[]): unknown {
	const missing = (fallback: unknown) => {
		...
		if (step.on_missing === "error")
			throw new Error(`on_missing: error at op "${step.op}"`);
		if (typeof step.on_missing === "object" && "literal" in step.on_missing)
	...
	switch (step.op) {
		...
		case "sub": {
			...
			return args
				.slice(1)
				.reduce<number>((acc, val) => acc - Number(val), Number(args[0]));
		}
		case "mul":
		...
		case "str_contains": {
			...
			if (
				...
				(lastArg.toLowerCase() === "all" || lastArg.toLowerCase() === "any")
			) {
