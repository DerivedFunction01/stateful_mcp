export async function validateStateReferences(
	...
	if (schema["x-mcp-ref"]) {
		...
		if (refType === "filter" && stores.filter) {
			...
			if (!exists) {
				throw new Error(
					`Invalid reference: Property value "${idOrAlias}" does not point to a valid filter in session.`,
				);
			}
			...
			if (!exists) {
				throw new Error(
					`Invalid reference: Property value "${idOrAlias}" does not point to a valid object in session.`,
				);
			}
			...
			if (!exists) {
				throw new Error(
					`Invalid reference: Property value "${idOrAlias}" does not point to a valid form in session.`,
				);
			}
		}
	}
