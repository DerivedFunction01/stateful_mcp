...
export function executeQuery(dataset: any[], query: QueryDefinition): any[] {
				...
				query.aggregations.forEach((agg) => {
					...
					switch (agg.function) {
						...
						case "max":
							...
							break;
						case "std_dev":
							resultRow[agg.alias] = Number(
								stdDev(values.map(Number)).toFixed(4),
							);
							break;
						case "median":
							resultRow[agg.alias] = Number(
								percentile(values.map(Number), 50).toFixed(4),
							);
							break;
						case "q1":
							resultRow[agg.alias] = Number(
								percentile(values.map(Number), 25).toFixed(4),
							);
							break;
						case "q3":
							resultRow[agg.alias] = Number(
								percentile(values.map(Number), 75).toFixed(4),
							);
							break;
						case "range": {
							const nums = values.map(Number);
							resultRow[agg.alias] =
								nums.length > 0 ? Math.max(...nums) - Math.min(...nums) : 0;
							break;
						}
					}
				});
	...
	create: async (options) => {
		...
		if (options.data) {
			...
			try {
				...
				console.error(
					`MemoryQueryEngine failed to load fixture from ${options.fixture}:`,
					err,
				);
			}
		}
