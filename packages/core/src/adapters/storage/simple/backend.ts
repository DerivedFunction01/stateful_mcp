...
export interface KvBackend {
  getSessionState(sessionId: string, id: string): Promise<Record<string, any> | null>;
  setSessionState(sessionId: string, id: string, value: Record<string, any>): Promise<void>;
  ...
  getPersistentState(id: string, scope: OwnerScope): Promise<Record<string, any> | null>;
  setPersistentState(id: string, scope: OwnerScope, value: Record<string, any>): Promise<void>;
  deletePersistentState(id: string, scope: OwnerScope): Promise<void>;
  scanPersistentStates(scope: OwnerScope, includeGlobal?: boolean): AsyncIterable<Record<string, any>>;
...
export class MemoryKvBackend implements KvBackend {
  ...
  getSessionState(sessionId: string, id: string): Promise<Record<string, any> | null> {
    return Promise.resolve(this.sessionStates.get(sessionKey(sessionId, id)) ?? null);
  }
  ...
  async *scanSessionStates(sessionId: string): AsyncIterable<Record<string, any>> {
    const prefix = sessionPrefix(sessionId);
  ...
  getPersistentState(id: string, scope: OwnerScope): Promise<Record<string, any> | null> {
    return Promise.resolve(this.persistentStates.get(persistentKey(id, scope)) ?? null);
  }
  ...
  async *scanPersistentStates(scope: OwnerScope, includeGlobal?: boolean): AsyncIterable<Record<string, any>> {
    ...
    for (const [key, val] of this.persistentStates.entries()) {
      if (prefix && key.startsWith(prefix)) {
        yield val;
      } else if (includeGlobal && scope.level === "user" && key.startsWith(globalPrefix)) {
        yield val;
  ...
  getAlias(sessionId: string, alias: string): Promise<string | null> {
    return Promise.resolve(this.aliases.get(aliasKey(sessionId, alias)) ?? null);
  }
...
export class JsonlKvBackend implements KvBackend {
  private sessionStates = new Map<string, { value: Record<string, any>; sessionId: string }>();
  private persistentStates = new Map<string, { value: Record<string, any>; scope: OwnerScope }>();
  private aliases = new Map<string, string>();
  ...
  private async truncateAndWrite(filePath: string, lines: string[]): Promise<void> {
    await this.enqueueWrite(async () => {
      await this.ensureDir(filePath);
      await fs.writeFile(
        filePath,
    ...
    try {
      if (await this.fileOrDirExists(this.sessionFilePath)) {
        ...
        for (const line of raw.split("\n")) {
          ...
          if (entry.type === "state") {
            this.sessionStates.set(entry.data[Object.keys(entry.data).find(k => k.endsWith("Id")) || "id"], {
              value: entry.data,
  ...
  async getSessionState(sessionId: string, id: string): Promise<Record<string, any> | null> {
    await this.initSessions();
    const found = this.sessionStates.get(id);
    return found?.value ?? null;
  }

  async setSessionState(sessionId: string, id: string, value: Record<string, any>): Promise<void> {
    await this.initSessions();
    this.sessionStates.set(id, { value, sessionId });
    if (this.sessionFilePath) {
      await this.appendLine(this.sessionFilePath, JSON.stringify({ type: "state", sessionId, data: value }));
    }
  }
  ...
  async listSessionIds(sessionId: string): Promise<string[]> {
    ...
    return Array.from(this.sessionStates.values())
      ...
      .map((v) => {
        const state = v.value;
        return String(state[Object.keys(state).find((k) => k.endsWith("Id")) || Object.keys(state)[0]!]);
      });
  }
  ...
  async getPersistentState(id: string, scope: OwnerScope): Promise<Record<string, any> | null> {
    await this.initPersistent();
    for (const [key, entry] of this.persistentStates.entries()) {
      if (key === id) {
        if (entry.scope.level === "global") return entry.value;
        if (scope.level === "user" && entry.scope?.userId === scope.userId) return entry.value;
      }
    }
    return null;
  }

  async setPersistentState(id: string, scope: OwnerScope, value: Record<string, any>): Promise<void> {
    await this.initPersistent();
    this.persistentStates.set(id, { value, scope });
    if (this.persistentFilePath) {
      await this.appendLine(
        this.persistentFilePath,
        JSON.stringify({ type: "persistent_state", id, data: { ...value, scope } }),
      );
    }
  }
  ...
  async *scanPersistentStates(scope: OwnerScope, includeGlobal?: boolean): AsyncIterable<Record<string, any>> {
    ...
    for (const entry of this.persistentStates.values()) {
      if (entry.scope.level === "global") {
        ...
        if (includeGlobal) {/* already yielded above */}
      }
    }
  }
  ...
  private async serializeAllSessions(): Promise<void> {
    ...
    for (const [key, targetId] of this.aliases.entries()) {
      ...
      lines.push(JSON.stringify({ type: "alias", sessionId: key.slice(0, colon), alias: key.slice(colon + 1), targetId }));
    }
  ...
  private async serializeAllPersistent(): Promise<void> {
    ...
    for (const [id, { value, scope }] of this.persistentStates.entries()) {
      lines.push(JSON.stringify({ type: "persistent_state", id, data: { ...value, scope } }));
    }
