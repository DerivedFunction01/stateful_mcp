...
import { QueryCompiler, type SqlDialect } from "../../../translation/sql-compiler";
...
export class SqlBackend {
  ...
  static async connect(dialect: SqlDialect, target: string): Promise<SqlBackend> {
    switch (dialect) {
      case "sqlite": {
      ...
      case "postgres":
        return new SqlBackend("postgres", new Pool({ connectionString: target }));
      default: {
  ...
  async query(sql: string, params: any[] = []): Promise<Record<string, any>[]> {
    switch (this.dialect) {
      case "sqlite":
        return (this.conn as Database).query(sql).all(...params) as Record<string, any>[];
      case "duckdb": {
        const reader = await (this.conn as DuckDBConnection).runAndReadAll(sql, params);
