...
import { SqlBackend, type SqlStatement } from "./backend";
import type { EntityConfig, ChildTableConfig } from "./entity-config";
...
export class GenericSqlEntityStore<Session, Persistent> {
  ...
  async setSession(sessionId: string, id: string, state: Session): Promise<void> {
    ...
    const statements: SqlStatement[] = [{ sql: upsert.sql, params: upsert.params }];
  ...
  ): Promise<Array<Persistent & { scope: { level: string; userId?: string } }>> {
    const conditions = this.buildScopeConditions(scope, includeGlobal);
    const sel = this.backend.compiler.compileSelect({
      table: this.config.savedTable,
      select: [{ column: "id" }, { column: "scope_level" }, { column: "user_id" }],
      where: conditions,
    });
    const rows = await this.backend.query(sel.sql, sel.params);
    const results: Array<Persistent & { scope: { level: string; userId?: string } }> = [];
    for (const r of rows) {
  ...
  async setAlias(sessionId: string, alias: string, targetId: string): Promise<void> {
    const upsert = this.backend.compiler.compileReplace({
      table: this.config.aliasTable,
      values: { session_id: sessionId, alias_name: alias, target_id: targetId } as Record<string, any>,
  ...
  private async assembleSession(row: Record<string, any>): Promise<Session> {
    let state = this.config.rowToSession(row);
    for (const child of this.config.children ?? []) {
      const csel = this.backend.compiler.compileSelect({
        table: child.table,
        where: [{ column: child.parentIdColumn, op: "eq", value: row[this.config.idField] }],
  ...
  private async assemblePersistent(
    ...
    let state = this.config.rowToPersistent(row, savedRow);
    for (const child of this.config.children ?? []) {
      const csel = this.backend.compiler.compileSelect({
        table: child.table,
        where: [{ column: child.parentIdColumn, op: "eq", value: row[this.config.idField] }],
