...
export async function runLogTests() {
	...
	const f1 = await filterStore.add(
		fRoot,
		[{ property: "price", operator: "gt", value: 100 }],
		sessionId,
	);
	const f2 = await filterStore.add(
		f1,
		[{ property: "category", operator: "eq", value: "apparel" }],
		sessionId,
	);
	const f3 = await filterStore.add(
		f2,
	...
	if (
		page1.entries.length !== 2 ||
		page1.entries[0].id !== f3 ||
		page1.entries[1].id !== f2
	) {
	...
	if (
		page2.entries.length !== 2 ||
		page2.entries[0].id !== f1 ||
		page2.entries[1].id !== fRoot
	) {
		throw new Error(`Page 2 mismatch: ${JSON.stringify(page2)}`);
	}
	if (page2.next_page_token !== null || page2.has_more !== false) {
		throw new Error(
			"Expected page 2 to reach the root and have no next page token",
		);
	}
	...
	const o1 = await objectStore.set(
		oRoot,
		["title"],
		"Holiday Party",
		sessionId,
	...
	async function logOpenObjects(id: string, limit: number) {
		...
		while (currentNodeId && count < limit) {
			...
			const parentNode = node.parentObjectId
				? await objectStore.getObject(node.parentObjectId, sessionId)
				: null;
			...
			for (const key of Object.keys(currData)) {
				if (
					!parentNode ||
					JSON.stringify(currData[key]) !== JSON.stringify(parentData[key])
				) {
	...
	if (
		oPage1.entries.length !== 2 ||
		oPage1.entries[0].id !== o2 ||
		oPage1.entries[1].id !== o1
	) {
	...
	if (Object.keys(oPage1.entries[0].delta).join(",") !== "start_date") {
		throw new Error(
			`Expected delta for o2 to be start_date, got: ${JSON.stringify(oPage1.entries[0].delta)}`,
		);
	}
	if (Object.keys(oPage1.entries[1].delta).join(",") !== "title") {
		throw new Error(
			`Expected delta for o1 to be title, got: ${JSON.stringify(oPage1.entries[1].delta)}`,
		);
	}
	console.log(
		"✓ Object stateful log traversal and sparse delta extraction verified successfully.",
	);
	...
	try {
		...
		throw new Error(
			"Expected verifyToken to throw an error for tampered token",
		);
	} catch (err: any) {
		// Expected behavior
	}
	console.log(
		"✓ Cryptographic log session signature security check verified successfully.",
	);
}
