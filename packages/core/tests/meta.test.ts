...
export async function runMetaTests() {
  ...
  if (!middlewareAbout.includes("Middleware Architecture")) {
    throw new Error(`Expected default middleware about content, got: ${middlewareAbout}`);
  }
  ...
  await fs.writeFile(customFile, "# Custom Firm About\nEnforcing strict enterprise policies.");
  ...
  if (!customAbout.includes("Custom Firm About") || customAbout.includes("Middleware Architecture")) {
  ...
  const concatenated = await resolveAboutOrExamples(locators, "config/examples/filter.md", workspaceRoot);
  if (!concatenated.includes("Example One content") || !concatenated.includes("Example Two content")) {
    throw new Error(`Expected both examples concatenated, got: ${concatenated}`);
  }

  // Verify manual page pagination
  const parts = concatenated.split("\n\n---\n\n");
  if (parts.length !== 2 || parts[0] !== "Example One content" || parts[1] !== "Example Two content") {
