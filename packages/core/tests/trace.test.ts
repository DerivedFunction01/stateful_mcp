...
describe("TraceStore Engine Tests", () => {
  ...
  test("recordTrace normalizes step IDs with auto-incrementing suffixes", () => {
    const rawForm: TraceForm = {
      ...
      steps: [
        ...
        { action: "filter_add_rule", args: { field: "dept", op: "eq", value: "$input.dept" } },
        { action: "filter_add_rule", args: { field: "status", op: "eq", value: "active" } }
      ]
    };
  ...
  test("executeTrace executes steps with variable bindings and AST conditions", async () => {
    store.recordTrace({
      ...
      input_slots: {
        cpu_pct: { type: "number", description: "Current CPU percentage", required: true }
      },
      steps: [
        {
          id: "check_cpu",
          ...
          conditions: [
            {
              pipeline: [
                { op: "get", args: [{ "$init": "response" }, "cpu"], return_var: "cpu" },
    ...
    const executor = async (action: string, args: Record<string, any>) => {
      ...
      if (action === "scale_up") return { scaled: true, replicas: args.replicas };
      return { idle: true };
    };

    const res = await store.executeTrace("system_scale", { cpu_pct: 92 }, executor);
    expect(res.status).toBe("completed");
    expect(res.step_results?.["scale_up"]).toEqual({ scaled: true, replicas: 5 });
  });
  ...
  test("executeTrace pauses when step has autonomous=false and resumes via resumeTrace", async () => {
    store.recordTrace({
      ...
      steps: [
        ...
        { id: "s2", action: "transfer_funds", autonomous: false, args: { target: "ACC-123" } }
      ]
    });
    ...
    const resumed = await store.resumeTrace(res.resume_token!, { transfer_id: "TX-999" });
