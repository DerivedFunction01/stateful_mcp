...
export async function runEventTests() {
	...
	try {
		await eventStore.init("observation", sessionId, "init_invalid", [
			{ type: "HeartRate" } as any,
		]); // missing value
		throw new Error(
			"Expected schema validation to fail for invalid initial data",
		);
	} catch (err: any) {
		if (err.code !== "OBJECT_VALIDATION_FAILED") {
			throw new Error(
				`Expected OBJECT_VALIDATION_FAILED code, got ${err.code || err.message}`,
			);
		}
	}
	...
	const initCommitId = await eventStore.init(
		"observation",
		sessionId,
		aliasInit,
	...
	if (initCommitId !== aliasInit) {
		throw new Error(
			`Expected alias ${aliasInit} to be returned, got ${initCommitId}`,
		);
	}
	...
	if (
		...
		initialArray[0]?.type !== "HeartRate" ||
		initialArray[0]?.value !== 72
	) {
		throw new Error(
			`Baseline projection mismatch: ${JSON.stringify(initialArray)}`,
		);
	}
	console.log(
	...
	if (batch.eventIds.length !== 2)
		throw new Error("Expected two batch event IDs");
	const batchArray = await eventStore.project("batch_tip", sessionId);
	if (
		!batchArray.some((event) => event.type === "BatchA") ||
		!batchArray.some((event) => event.type === "BatchB")
	) {
	...
	if (
		retry.commitId !== retryAgain.commitId ||
		retry.eventIds.join() !== retryAgain.eventIds.join()
	)
		throw new Error("Batch retry must be idempotent");
	try {
		await eventStore.appendBatch(
			sessionId,
			"batch_tip",
			[{ type: "Valid", value: 3 }, { type: "Invalid" }],
			"invalid_batch",
		);
	...
	if (await eventStore.getCommit("invalid_batch", sessionId))
		throw new Error("Invalid batch must not create a commit");
	console.log(
		"✓ Batch validation and single-commit projection verified successfully.",
	);
	...
	// Patch Temp event
	const tip2 = await eventStore.patch(
		sessionId,
		"t1",
		tempEvent.event_id,
		{ value: 99.1, comment: "Slight fever" },
		"t2",
	);
	...
	if (
		!patchedTemp ||
		patchedTemp.value !== 99.1 ||
		patchedTemp.comment !== "Slight fever"
	) {
		throw new Error(
			`Expected patched Temp event values, got: ${JSON.stringify(patchedTemp)}`,
		);
	}

	// Delete Temp event
	const tip3 = await eventStore.delete(
		sessionId,
		"t2",
		tempEvent.event_id,
		"t3",
	...
	// Create baseline
	const base = await mergeStore.init(
		"observation",
		sessionMerge,
		"main_branch",
	...
	if (!heartRateEventId)
		throw new Error("Expected heart rate event ID to exist");

	// Create Stream A (branches from main_branch, updates HeartRate)
	const tipA = await mergeStore.patch(
		sessionMerge,
		"main_branch",
		heartRateEventId,
		{ value: 85 },
	...
	const tipB = await mergeStore.patch(
		sessionMerge,
		"main_branch",
		heartRateEventId,
		{ value: 90 },
	...
	// Attempt N-way merge
	const mergeRes = await mergeStore.merge(
		sessionMerge,
		["stream_a", "stream_b"],
		"main_branch",
	);
	if (mergeRes.status !== "conflict" || !mergeRes.merge_session_id) {
		throw new Error(
			`Expected merge conflict, got: ${JSON.stringify(mergeRes)}`,
		);
	}

	const sessionInfo = await mergeStore.mergeInspect(mergeRes.merge_session_id);
	if (
		sessionInfo.conflicts.length !== 1 ||
		sessionInfo.conflicts[0]?.event_id !== heartRateEventId
	) {
		throw new Error(
			`Unexpected conflict details: ${JSON.stringify(sessionInfo)}`,
		);
	}
	...
	const finalMergedCommit = await mergeStore.mergeCommit(
		resolvedSessionId,
		sessionMerge,
	);
	...
	if (finalArray.length !== 1 || finalArray[0]?.value !== 90) {
		throw new Error(
			`Expected merged heart rate value to be 90, got: ${JSON.stringify(finalArray)}`,
		);
	}
	console.log(
		"✓ N-way conflict detection and stateful resolution verified successfully.",
	);
	...
	const step1 = await gcStore.append(
		gcSessionId,
		"dev_baseline",
		{ type: "Temp", value: 98.6 },
		"dev_alias",
	);
	const step2 = await gcStore.append(
		gcSessionId,
		"dev_alias",
		{ type: "O2", value: 98 },
		"dev_alias",
	...
	const step3 = await gcStore.append(
		gcSessionId,
		"dev_alias",
		{ type: "BP", value: 120 },
		"dev_alias",
	...
	if (projectedGc.length !== 4) {
		throw new Error(
			`Expected 4 projected events after auto-compression, got ${projectedGc.length}`,
		);
	}
	...
	if (gcRes.deleted_count < 2) {
		throw new Error(
			`Expected squashed linear commits to be deleted by GC, deleted count: ${gcRes.deleted_count}`,
		);
	}
	...
	const persisted = await persistentStore.get(initCommitId, {
		level: "global",
	});
	if (
		!persisted ||
		persisted.description !== "Initial baseline" ||
		persisted.tags[0] !== "tag1"
	) {
	...
	if (
		!persisted2 ||
		persisted2.description !== "Patched temp observation" ||
		persisted2.parentCommitId !== null
	) {
		throw new Error(
			`Persisted state mismatch or not compressed: ${JSON.stringify(persisted2)}`,
		);
	}
