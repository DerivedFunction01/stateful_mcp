...
import {
	executeCommandMacroGraph,
	executeCommandMacroPlans,
} from "../src/engine/command-macro-executor";
import {
	getCommandMacroAutocomplete,
...
} from "../src/parser/command/command-macro-authoring-template";
import { bindCommandMacro } from "../src/parser/command/command-macro-binder";
import {
	evaluateMacroBoundary,
	evaluateMacroEnvelope,
} from "../src/parser/command/command-macro-boundary";
import {
	planCommandMacroBatch,
	validateMacroCompositionGraph,
} from "../src/parser/command/command-macro-graph";
...
const macro: ParserCommandMacro = {
	...
	active: true,
	root: {
		roleName: "subjective.presenting_complaint",
	...
	arguments: [
		{
			argumentId: "complaint",
			name: "complaint",
			roleName: "subjective.presenting_complaint",
			position: 0,
			target: { targetSchema: "ObservationEvent", targetPath: "concept" },
			extraction: { kind: "concept", requireConceptFilter: true },
		},
		{
			argumentId: "severity",
			name: "severity",
			roleName: "subjective.severity",
			position: 1,
			target: { targetSchema: "ObservationEvent", targetPath: "severity" },
			extraction: {
				kind: "scalar",
				valueType: "integer",
				extraction: { pattern: "^\\d+$", fullSpan: true },
			},
		},
	],
};
...
describe("command macro v2 foundation", () => {
	test("lexes quoted, named, and grouped values without split semantics", () => {
		const result = lexCommandMacro(
			'^cc complaint="air, problem" qualifiers=[one; two]',
		);
		expect(result.macroName).toBe("cc");
		expect(result.arguments.map((token) => token.argumentId)).toEqual([
			"complaint",
			"qualifiers",
		]);
	...
	test("binds direct operations and never creates ParsedItem", () => {
		const result = bindCommandMacro("^cc SOB 4", macro, {
			groupId: "g1",
			cellRef: "c1",
		});
	...
	test("rejects missing named groups and invalid full-span contracts", () => {
		const diagnostics = validateParserCommandMacro({
			...macro,
			arguments: [
				{
					...macro.arguments[1]!,
		...
		expect(
			diagnostics.some((item) => item.message.includes("required named group")),
		).toBe(true);
		expect(diagnostics.some((item) => item.message.includes("anchored"))).toBe(
			true,
		);
	});
	...
	test("persists command macros separately from prose macros", async () => {
		...
		expect(await store.get("cc")).toMatchObject({
			macroId: "cc-v1",
			macroName: "cc",
		});
	});

	test("stores command render templates separately and renders generic pipeline conditions", async () => {
		const store = new KvCommandTemplateStore(new MemoryKvBackend());
		await store.set({
			templateId: "cc-preview",
			stage: "preview",
			templateText: "{complaint}{severity}",
			slots: {
				complaint: { sourcePath: "arguments.complaint.value" },
				severity: {
					sourcePath: "arguments.severity",
					format: " (severity {value})",
					conditions: {
						pipeline: [
							{
								op: "eq",
								args: [{ $path: ["status"] }, { $literal: "assigned" }],
							},
						],
					},
				},
		...
		const template = await store.getById("cc-preview");
		expect(template?.stage).toBe("preview");
		expect(
			renderCommandMacroTargets(template!, {
				data: {
					arguments: {
						complaint: { value: "SOB", status: "assigned" },
						severity: { value: 4, status: "assigned" },
					},
				},
			}).text,
		).toBe("SOB (severity 4)");
	...
	test("compiles command template SQL with scoped lookup index", () => {
		const compiler = new CommandTemplateQueryCompiler("sqlite");
		const ddl = compiler.getTableDDL("command_templates");
		expect(ddl[0]?.sql).toContain("templateId");
		expect(ddl[1]?.sql).toContain("macroId");
		expect(
			compiler.compileGetQuery("cc-preview", "command_templates").params,
		).toContain("cc-preview");
	});

	test("composes repeated child render results through a parent slot", () => {
		const result = renderCommandMacroTargets(
			{
				templateId: "parent",
				stage: "confirmation",
				...
				slots: {
					children: {
						sourcePath: "children.qualifier",
						child: { childMacroName: "qualifier", mode: "inline" },
						listOptions: { delimiter: ", ", lastDelimiter: " and " },
					},
				},
			},
			{
				children: {
					qualifier: [
						{ text: "exertional", status: "resolved" },
						{ text: "intermittent", status: "resolved" },
					],
				},
			},
		);
		expect(result.text).toBe("exertional and intermittent");
		expect(result.status).toBe("resolved");
	});

	test("rejects invalid command template pipelines and unresolved tokens", () => {
		const diagnostics = validateCommandTemplate({
			templateId: "invalid",
			stage: "preview",
			...
			slots: {
				known: {
					sourcePath: "value",
					conditions: {
						pipeline: [{ op: "not-a-real-op" as never, args: [] }],
					},
				},
			},
		});
		expect(
			diagnostics.some((item) => item.includes("no slot definition")),
		).toBe(true);
		expect(
	...
	test("validates the full plan before mutation and compensates failures", async () => {
		...
		const result = await executeCommandMacroPlans(
			[
				{
					cellRef: "c",
					targetSchema: "ObservationEvent",
			...
			],
			{
				apply: async (operation) => {
					...
					if (operation.operationId === "two") throw new Error("write failed");
				},
				rollback: async (operation) => {
					rolledBack.push(operation.operationId);
				},
			},
		);
	...
	test("uses structured slots and preserves bracket expressions as literals", () => {
		const rendered = renderCommandMacroTemplate({
			version: 1,
			parts: [
		...
		expect(rendered.slots[0]).toMatchObject({
			slotId: "duration",
			status: "empty",
		});
	});
	...
	test("rejects authoring slots that do not reference macro arguments", () => {
		const diagnostics = validateParserCommandMacro({
			...
			authoringTemplate: {
				version: 1,
				parts: [{ kind: "slot", slotId: "custom-expression", occurrence: 0 }],
			},
		});
		expect(
			diagnostics.some((item) => item.message.includes("unknown argument")),
		).toBe(true);
	});
	...
	test("advances structured slot state after an autocomplete assignment", () => {
		...
		const assignment = assignMacroSlot(
			template,
			new Map(),
			{ slotId: "complaint", occurrence: 0 },
			"SOB",
		);
		expect(assignment.rendered.text).toBe(
			"^cc complaint=SOB duration=<duration>",
		);
		expect(assignment.activeSlot).toMatchObject({
			slotId: "duration",
			status: "empty",
		});
	});
		...
		const result = renderCommandMacroTargets(
			{
				templateId: "preview",
				stage: "preview",
				templateText: "{complaint}{duration}",
				slots: {
					complaint: {
						sourcePath: "arguments.complaint.value",
						format: "{display}",
					},
					duration: {
						sourcePath: "arguments.duration",
						conditions: {
							pipeline: [
								{
									op: "eq",
						...
						format: " for {value.value} {value.unit}",
					},
				},
			},
			{
				data: {
					arguments: {
						complaint: {
							value: { display: "Shortness of breath" },
							status: "assigned",
						},
						duration: {
		...
		const result = renderCommandMacroTargets(
			{
				templateId: "literal",
				stage: "preview",
				templateText: "custom expression [__]",
				slots: {},
			},
			{ data: {} },
		);
	...
	test("plans explicit parent-child links and merge strategies", async () => {
		const parent: ParserCommandMacro = {
			...
			children: [
				{
					childMacroName: "qualifier",
					parentRoleName: "subjective.presenting_complaint",
					parentTargetPath: "qualifiers",
		...
		const result = await planCommandMacroBatch(
			"^parent SOB\n^qualifier diaphoresis",
			store,
			{ groupId: "batch-1" },
		);
		expect(result.diagnostics).toEqual([]);
		expect(result.graph?.links[0]).toMatchObject({
			mergeStrategy: "append",
			parentTargetPath: "qualifiers",
		});
		expect(result.graph?.plans[1]?.parentRef).toBe(
			result.graph?.plans[0]?.cellRef,
		);
	});

	test("rejects cyclic child composition before execution", () => {
		const a: ParserCommandMacro = {
			...macro,
			macroName: "a",
			children: [
				{
					childMacroName: "b",
					parentRoleName: "subjective.presenting_complaint",
					parentTargetPath: "child",
					mergeStrategy: "replace",
				},
			],
		};
		const b: ParserCommandMacro = {
			...macro,
			macroName: "b",
			children: [
				{
		...
		const diagnostics = validateMacroCompositionGraph(
			a,
			new Map([
				["a", a],
				["b", b],
	...
	test("enforces macro envelopes and reports boundary evidence", () => {
		const envelope = evaluateMacroEnvelope("^cc SOB one two three four", 3, {
			maxWords: 2,
		});
		expect(envelope.accepted).toBe(false);
		expect(envelope.reasons[0]).toContain("word distance");
		const local = evaluateMacroBoundary(
			"shoulder pain on the right",
			{ start: 20, end: 25 },
			{ start: 0, end: 7 },
			{ direction: "right", maxWords: 4, unit: "words" },
		);
		expect(local.accepted).toBe(true);
	});

	test("keeps a macro from consuming a distant paragraph", () => {
		const bounded: ParserCommandMacro = {
			...macro,
			boundary: { maxParagraphs: 0, maxWords: 8 },
		};
		const result = lexCommandMacro(
			"^cc SOB\n\nfar away prose that must not be consumed",
			bounded,
		);
		expect(
			result.diagnostics.some((item) =>
				item.message.includes("macro envelope"),
			),
		).toBe(true);
	});
	...
	test("compiles command macro DDL and context-aware queries", () => {
		...
		const query = compiler.compileGetQuery("cc", "parser_command_macros", {
			personnelId: "p1",
			profileId: "profile",
		});
	...
	test("requires an explicit prose argument for a prose boundary", () => {
		...
		expect(
			result.diagnostics.some((item) =>
				item.message.includes("prose argument"),
			),
		).toBe(true);
	});
		...
		const proseMacro: ParserCommandMacro = {
			...
			arguments: [
				...macro.arguments,
				{
					argumentId: "history",
					name: "history",
					roleName: "history.context",
					target: { targetSchema: "History", targetPath: "text" },
					extraction: {
						kind: "prose",
	...
	test("exposes static compatible macro candidates without learning scores", async () => {
		const current: ParserCommandMacro = {
			...macro,
			children: [
				{
					childMacroName: "qualifier",
		...
		const compatible: ParserCommandMacro = {
			...macro,
			macroId: "qualifier-v1",
			macroName: "qualifier",
		};
		const unrelated: ParserCommandMacro = {
			...macro,
			macroId: "other-v1",
			macroName: "other",
			root: { ...macro.root, targetSchema: "OtherEvent" },
		};
		const store = new KvParserCommandMacroStore(new MemoryKvBackend());
		...
		expect(
			result.find((item) => item.macro.macroName === "qualifier")
				?.compatibility,
		).toBe("declared-child");
	});
	...
	test("persists generated cells with macro provenance and compensates on failure", async () => {
		...
		const cellStore = {
			...
			save: async (cell: Cell) => {
				saved.set(cell.cellId, structuredClone(cell));
			},
		...
		const cell = {
			cellId: "macro-cell",
			sessionId: "session",
			collection: { kind: "notebook", collectionId: "session" },
			intentKind: "macro_command",
			mode: "macro",
			rawInput: "^cc SOB 4",
			...
			context: { objects: {} },
			macro: { batchId: "batch", definitionIds: [], status: "pending_commit" },
		} as Cell;
		const result = await processor.execute(cell);
		expect(result.error).toBeUndefined();
		expect(result.cell.macro?.generatedCellIds).toHaveLength(1);
		const generated = await cellStore.get(
			result.cell.macro!.generatedCellIds![0]!,
		);
		expect(generated?.metadata).toMatchObject({
			macroGenerated: true,
			sourceMacroCellId: "macro-cell",
		});
