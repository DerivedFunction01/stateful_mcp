...
describe("V2 durable macro stores", () => {
	it("persists and filters macro definitions through KV", async () => {
		...
		expect((await store.get("primary_diagnosis"))?.macroId).toBe("v2-primary-diagnosis-1");
		expect((await store.list()).map((macro) => macro.macroName)).toEqual(["primary_diagnosis"]);
	});
	...
	it("compiles SQL DDL and AST queries without string-built SQL", () => {
		...
		const query = compiler.getQuery("primary_diagnosis", "v2_macros", { profileId: "default" });
