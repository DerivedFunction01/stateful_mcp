...
import type { ParserSyntaxProfile, AttributeParserRule } from "../src/store/interfaces";
...
await conceptStore.addNamespace({ code: "LOINC", description: "LOINC", isPublic: true, isExternalPrivate: false });
await conceptStore.addNamespace({ code: "SNOMED", description: "SNOMED", isPublic: true, isExternalPrivate: false });
await conceptStore.addNamespace({ code: "RxNorm", description: "RxNorm", isPublic: true, isExternalPrivate: false });

await conceptStore.addConcept({ id: "SNOMED::2235008", standardCode: "2235008", display: "Low back pain", namespaceCode: "SNOMED", active: true });
await conceptStore.addConcept({ id: "LOINC::8310-5", standardCode: "8310-5", display: "Body temperature", namespaceCode: "LOINC", active: true });
await conceptStore.addConcept({ id: "SNOMED::19213003", standardCode: "19213003", display: "Diaphoresis", namespaceCode: "SNOMED", active: true });
await conceptStore.addConcept({ id: "SNOMED::80436009", standardCode: "80436009", display: "Palpitations", namespaceCode: "SNOMED", active: true });
await conceptStore.addConcept({ id: "SNOMED::422587007", standardCode: "422587007", display: "Nausea", namespaceCode: "SNOMED", active: true });
await conceptStore.addConcept({ id: "RxNorm::723", standardCode: "723", display: "Amoxicillin", namespaceCode: "RxNorm", active: true });

const expressions: CustomExpression[] = [
{ id: "1", term: "Low back pain", regexPattern: "\\bback pain\\b", isCaseInsensitive: true, targetAssignment: "MAIN_TERM", conceptId: "SNOMED::2235008", priorityWeight: 1, active: true },
{ id: "2", term: "Body temperature", regexPattern: "\\bfever\\b", isCaseInsensitive: true, targetAssignment: "MAIN_TERM", conceptId: "LOINC::8310-5", priorityWeight: 1, active: true },
{ id: "3", term: "Diaphoresis", regexPattern: "\\bsweating\\b", isCaseInsensitive: true, targetAssignment: "MAIN_TERM", conceptId: "SNOMED::19213003", priorityWeight: 1, active: true },
{ id: "4", term: "Palpitations", regexPattern: "\\bpalpitations\\b", isCaseInsensitive: true, targetAssignment: "MAIN_TERM", conceptId: "SNOMED::80436009", priorityWeight: 1, active: true },
{ id: "5", term: "Nausea", regexPattern: "\\bnausea\\b", isCaseInsensitive: true, targetAssignment: "MAIN_TERM", conceptId: "SNOMED::422587007", priorityWeight: 1, active: true },
{ id: "6", term: "Amoxicillin", regexPattern: "\\bamoxicillin\\b", isCaseInsensitive: true, targetAssignment: "MAIN_TERM", conceptId: "RxNorm::723", priorityWeight: 1, active: true },
];
...
anchorPattern: "History of Present Illness \\(HPI\\):(?<hpi_section>[\\s\\S]+?)(?=(?:Review of Systems|$))",
delegateTemplateId: "cardio_hpi",
},
{
slotName: "ros_section",
slotType: "sub_section",
anchorPattern: "Review of Systems \\(ROS\\):(?<ros_section>[\\s\\S]+)",
...
repeatPattern: "^(?<system>Constitutional|Cardiovascular):\\s*(?<findings>[^\\n]+)",
subTemplate: {
textGroup: "findings",
slots: [
{
...
const { parsedItems, consumedRanges, remnantSegments } = await parser.parse(rawProse);

// ── Verification of parsed items ───────────────────────────────────────
// 1. Back Pain (chief complaint)
const backPain = parsedItems.find((t) => t.concept?.[0]?.conceptId === "SNOMED::2235008");
expect(backPain).toBeDefined();
expect(backPain?.extractedData?.concept?.conceptId).toBe("SNOMED::2235008");
expect(backPain?.extractedData?.duration).toEqual({ magnitude: 3, unit: "day" });
		expect(backPain?.extractedData?.severity).toEqual({ score: 8, maxScore: 10 });

// 2. Constitutional Negative Items (Fever, Sweating)
const fever = parsedItems.find((t) => t.concept?.[0]?.conceptId === "LOINC::8310-5");
expect(fever).toBeDefined();
expect(fever?.extractedData?.certainty).toBe("refuted");

const sweating = parsedItems.find((t) => t.concept?.[0]?.conceptId === "SNOMED::19213003");
...
const palpitations = parsedItems.find((t) => t.concept?.[0]?.conceptId === "SNOMED::80436009");
...
const amox = parsedItems.find((t) => t.concept?.[0]?.conceptId === "RxNorm::723");
...
const nauseaRem = remnantSegments.find((r) => r.text.includes("reports mild nausea"));
expect(nauseaRem).toBeDefined();
expect(nauseaRem?.remnantContext?.targetSchema).toBe("ObservationEvent");
expect(nauseaRem?.remnantContext?.itemOverrides?.sourceType).toBe("patient_reported");
});
});
