...
describe("V2 temporal values", () => {
	it("recognizes configurable relative-day aliases", () => {
		...
		expect(recognizeTemporalExpression("tomorrow").expression).toEqual({ kind: "relative_day", offsetDays: 1 });
	...
	it("resolves relative days against an explicit anchor", () => {
		...
		expect(result.value?.time?.startDatetime?.assertedTimestampUtc).toBe("2026-08-03T04:00:00.000Z");
		expect(result.value?.time?.endDatetime?.assertedTimestampUtc).toBe("2026-08-04T04:00:00.000Z");
	});

	it("preserves relative mathematical semantics", () => {
		const expression = recognizeTemporalExpression("3 days ago").expression!;
		expect(resolveTemporalExpression(expression, anchor).value?.relativeEstimate).toEqual({ direction: "retrospective", firstValue: 3, precisionUnit: "day" });
	});
	...
	it("resolves configured frequency shorthand without parser dependencies", () => {
		expect(resolveFrequency({ alias: "BID", prn: true, eventAnchor: "with_meal", prnReason: { conceptId: "pain", display: "Pain" } })).toEqual({ cadenceType: "event_anchored", interval: undefined, rate: undefined, eventAnchor: "with_meal", isPrn: true, prnReason: { conceptId: "pain", display: "Pain" } });
	});
});
