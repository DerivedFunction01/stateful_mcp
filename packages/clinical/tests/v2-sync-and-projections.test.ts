...
describe("V2 sync engine", () => {
	...
	it("evaluates sync rules against a clinical document projection", () => {
		...
		const doc: ClinicalDocumentReadModel = {
			documentId: "doc-1", sessionId: "s1", patientId: "p1", status: "draft",
			amendmentNotes: [], version: 2, eventHead: "h1",
			records: {
				r1: { recordId: "r1", schemaName: "Observation", values: { concept: "BP", value: 120 }, version: 1 },
			},
		};
	...
	it("returns empty when no sync config is set", () => {
		...
		const doc: ClinicalDocumentReadModel = {
			documentId: "doc-1", sessionId: "s1", patientId: "p1", status: "draft",
			amendmentNotes: [], version: 1, records: {},
		};
...
describe("V2 ProjectionRegistry", () => {
	it("registers and invokes handlers matching committed participant kinds", async () => {
		...
		const calls: string[] = [];
		registry.register({ kind: "clinical_events", onCommitted: async () => { calls.push("clinical"); } });
		registry.register({ kind: "workspace_events", onCommitted: async () => { calls.push("workspace"); } });
		registry.register({ kind: "cells", onCommitted: async () => { calls.push("cells"); } });
	...
	it("throws on duplicate registration", () => {
		const registry = new ProjectionRegistry();
		const handler = { kind: "clinical_events" as const, onCommitted: async () => {} };
