...
const sampleInsertPlan: OrderedLearningInsertPlan = {
	...
	parsedItemJson: JSON.stringify({ tag: "pain", targetSchema: "ObservationEvent" }),
	priorAcceptCount: 1,
...
describe("compileOrderedLearningInsertQuery", () => {
	test("produces INSERT OR REPLACE for sqlite", () => {
		const result = compileOrderedLearningInsertQuery(sampleInsertPlan, "sqlite");
	...
	test("produces INSERT ... ON CONFLICT for postgres", () => {
		const result = compileOrderedLearningInsertQuery(sampleInsertPlan, "postgres");
	...
	test("produces INSERT OR REPLACE for duckdb", () => {
		const result = compileOrderedLearningInsertQuery(sampleInsertPlan, "duckdb");
		expect(result.sql).toContain("INSERT OR REPLACE INTO");
	});

	test("includes all 26 columns", () => {
		const result = compileOrderedLearningInsertQuery(sampleInsertPlan, "sqlite");
		const colCount = (result.sql.match(/\w+Id/g) || []).length +
			(result.sql.match(/patient\w+/g) || []).length +
			(result.sql.match(/recencyScore|contractValid|stalePreference|reviewRequired/g) || []).length;
...
describe("compileOrderedLearningCorrectionQuery", () => {
	test("produces UPDATE for sqlite", () => {
		const result = compileOrderedLearningCorrectionQuery(sampleCorrectionPlan, "sqlite");
	...
	test("sets reviewRequired=1 when replacement is provided", () => {
		...
		const result = compileOrderedLearningCorrectionQuery(planWithReplacement, "sqlite");
		expect(result.sql).toContain("reviewRequired = 1");
		expect(result.sql).toContain("parsedItem = ?");
	});

	test("uses $N placeholders for postgres", () => {
		const result = compileOrderedLearningCorrectionQuery(sampleCorrectionPlan, "postgres");
		expect(result.sql).toContain("$1");
		expect(result.sql).not.toContain("?");
	});
});
