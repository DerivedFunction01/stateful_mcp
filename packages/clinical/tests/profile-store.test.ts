...
describe("unified profile stores", () => {
	test("persists and lists profiles through the KV backend", async () => {
		...
		await store.set({ profileId: "numerical-default", kind: "numerical", payload: { dateTimeFormats: [] } });
		expect((await store.list()).map((profile) => profile.profileId)).toEqual(["command-default", "numerical-default"]);
