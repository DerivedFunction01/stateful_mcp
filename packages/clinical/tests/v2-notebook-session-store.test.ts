...
const record = { sessionId: "s1", cellOrder: ["c1"], activeCellId: "c1", commandHistory: [":help"], revision: 1, updatedAt: "2026-01-01T00:00:00.000Z" };
...
describe("V2 notebook session stores", () => {
	it("persists editor-only session state in KV with revision checks", async () => {
		...
		await expect(store.save({ ...record, revision: 2 }, 0)).rejects.toThrow(/revision mismatch/);
	...
	it("uses the SQL query compiler AST for session persistence", () => {
		...
		expect(compiler.getTableDDL("v2_notebook_sessions")[0]!.sql).toMatch(/CREATE TABLE/i);
		expect(compiler.getQuery("s1", "v2_notebook_sessions").params).toContain("s1");
	});
});
