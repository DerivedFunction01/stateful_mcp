...
function validatePattern(rule: CommandMacroPatternRule, path: string, diagnostics: CommandMacroValidationDiagnostic[]): void {
	let names: Set<string>;
	try {
		new RegExp(rule.pattern);
		names = groupNames(rule.pattern);
	} catch (error) {
		diagnostics.push({ path, message: `invalid regular expression: ${String(error)}` });
		return;
	}
	const contract = rule.namedGroupContract;
	for (const name of contract?.required ?? []) {
		if (!names.has(name)) diagnostics.push({ path, message: `required named group '${name}' is missing` });
	}
	for (const name of contract?.allowed ?? []) {
		if (!names.has(name)) diagnostics.push({ path, message: `allowed named group '${name}' is missing` });
	}
	for (const name of contract?.disallowed ?? []) {
		if (names.has(name)) diagnostics.push({ path, message: `disallowed named group '${name}' is present` });
	}
	if (rule.fullSpan && !/^\^.*\$$/.test(rule.pattern)) {
		diagnostics.push({ path, message: "full-span rules must be anchored with ^ and $" });
	}
	for (const exclusion of rule.exclusions ?? []) {
		try { new RegExp(exclusion.pattern, exclusion.caseSensitive ? "" : "i"); }
		catch (error) { diagnostics.push({ path: `${path}.exclusions`, message: `invalid exclusion pattern: ${String(error)}` }); }
	}
}

function validateBounds(bounds: NumericBounds | undefined, path: string, diagnostics: CommandMacroValidationDiagnostic[]): void {
	if (!bounds) return;
	if (bounds.min !== undefined && !Number.isFinite(bounds.min)) diagnostics.push({ path, message: "minimum must be finite" });
	if (bounds.max !== undefined && !Number.isFinite(bounds.max)) diagnostics.push({ path, message: "maximum must be finite" });
...
function validateSpec(spec: CommandMacroValueSpec, path: string, diagnostics: CommandMacroValidationDiagnostic[]): void {
	switch (spec.kind) {
		case "concept":
			for (const [index, rule] of (spec.patterns ?? []).entries()) validatePattern(rule, `${path}.patterns[${index}]`, diagnostics);
			return;
		case "enum": {
			const values = new Set<string>();
			for (const [index, value] of spec.values.entries()) {
				if (!value.value.trim() || values.has(value.value)) diagnostics.push({ path: `${path}.values[${index}]`, message: "enum values must be non-empty and unique" });
				values.add(value.value);
				for (const [ruleIndex, rule] of value.patterns.entries()) validatePattern(rule, `${path}.values[${index}].patterns[${ruleIndex}]`, diagnostics);
			}
			return;
		}
		case "measurement":
			validatePattern(spec.extraction, `${path}.extraction`, diagnostics);
			if (!spec.dimension.trim()) diagnostics.push({ path, message: "measurement dimension is required" });
			if (!spec.magnitudeGroup || !spec.unitGroup) diagnostics.push({ path, message: "measurement groups are required" });
			validateBounds(spec.bounds?.raw, `${path}.bounds.raw`, diagnostics);
			validateBounds(spec.bounds?.normalized, `${path}.bounds.normalized`, diagnostics);
			if (spec.units?.allowed && spec.units.denied && spec.units.allowed.some((unit) => spec.units?.denied?.includes(unit))) diagnostics.push({ path, message: "a unit cannot be both allowed and denied" });
			return;
		case "temporal":
		case "scalar":
			validatePattern(spec.extraction, `${path}.extraction`, diagnostics);
			if (spec.kind === "scalar") validateBounds(spec.bounds, `${path}.bounds`, diagnostics);
			return;
		case "array":
...
export function validateParserCommandMacro(macro: ParserCommandMacro): CommandMacroValidationDiagnostic[] {
	const diagnostics: CommandMacroValidationDiagnostic[] = [];
	if (!macro.macroId.trim()) diagnostics.push({ path: "macroId", message: "macroId is required" });
	...
	if (macro.authoringTemplate) {
		...
		for (const [index, part] of macro.authoringTemplate.parts.entries()) {
			...
			if (!isStructuredMacroSlot(part)) diagnostics.push({ path: `authoringTemplate.parts[${index}]`, message: "invalid structured slot" });
			if (!argumentIds.has(part.slotId)) diagnostics.push({ path: `authoringTemplate.parts[${index}]`, message: `slot references unknown argument '${part.slotId}'` });
			const key = `${part.slotId}:${part.occurrence}`;
			if (occurrences.has(key)) diagnostics.push({ path: `authoringTemplate.parts[${index}]`, message: `duplicate slot occurrence '${key}'` });
			occurrences.add(key);
		}
	}
	...
	for (const [index, argument] of macro.arguments.entries()) {
		...
		if (!argument.argumentId || ids.has(argument.argumentId)) diagnostics.push({ path, message: "argument IDs must be non-empty and unique" });
...
export function assertValidParserCommandMacro(macro: ParserCommandMacro): void {
	...
	if (diagnostics.length) throw new Error(`Invalid command macro: ${diagnostics.map((item) => `${item.path}: ${item.message}`).join("; ")}`);
}

export function normalizeParserCommandMacro(macro: ParserCommandMacro): ParserCommandMacro {
