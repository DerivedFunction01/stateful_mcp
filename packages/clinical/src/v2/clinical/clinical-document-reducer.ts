...
export function reduceClinicalEvents(
	records: readonly ClinicalEventRecord[],
): ClinicalDocumentReadModel {
	const initialized = records.find(
		(record) => record.payload.kind === "clinical_document_initialized",
	);
	if (
		!initialized ||
		initialized.payload.kind !== "clinical_document_initialized"
	)
		throw new Error("Clinical document initialization event is missing");
	...
	if (initialized.payload.initialState)
		document = {
			...document,
			records: {
				initial: {
					...
					schemaName: "Note",
					schemaVersion: 1,
					values: initialized.payload.initialState,
					version: 1,
				},
			},
		};
...
export function reduceClinicalEvent(
	...
	switch (event.kind) {
		...
		case "clinical_record_patched": {
			...
			if (!existing)
				throw new Error(`Clinical record '${event.recordId}' was not found`);
			existing.values = {
				...existing.values,
		...
		case "clinical_record_removed":
			if (next.records[event.recordId])
				next.records[event.recordId]!.removed = true;
			return next;
