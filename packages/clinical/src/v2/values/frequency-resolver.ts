...
export const V2_FREQUENCY_DEFAULTS: FrequencyProfile = {
  aliases: {
...
export function resolveFrequency(
  ...
  profile: FrequencyProfile = V2_FREQUENCY_DEFAULTS,
): FrequencyResolution {
  const shorthand = input.alias ? profile.aliases[input.alias] : undefined;
	const res: FrequencyResolution = {
		cadenceType: input.eventAnchor ? "event_anchored" : input.times && input.period ? "interval" : shorthand ? "interval" : "one_time",
		rate: !input.eventAnchor && input.times && input.period
