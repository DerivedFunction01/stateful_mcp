...
export function recognizeTemporalExpression(text: string, profile: V2TemporalSyntaxProfile = createV2TemporalSyntaxProfile({ profileId: "v2-temporal-default" })): { expression?: TemporalExpression; diagnostics: string[] } {
	...
	if (alias !== undefined) return { expression: { kind: "relative_day", offsetDays: alias }, diagnostics: [] };
	for (const delimiter of profile.rangeDelimiters) {
		...
		if (index > 0) {
			...
			const right = recognizeTemporalExpression(input.slice(index + delimiter.length), profile);
			if (left.expression && right.expression && !left.diagnostics.length && !right.diagnostics.length) return { expression: { kind: "range", start: left.expression, end: right.expression }, diagnostics: [] };
		}
	}
	for (const rule of profile.dateRecognitionRules) {
		...
		return { expression: { kind: "absolute_instant", instant: time ? `${year}-${month}-${day}T${time}` : `${year}-${month}-${day}T00:00:00.000Z`, precision: rule.precision }, diagnostics: [] };
	}
	const tokens = lower.split(/\s+/);
	const direction = profile.directionAliases[tokens[0] ?? ""] ?? profile.directionAliases[tokens.at(-1) ?? ""];
	const numericIndex = direction === "prospective" && profile.directionAliases[tokens[0] ?? ""] ? 1 : 0;
	...
	if (Number.isFinite(amount) && unit) return { expression: { kind: "relative", direction: direction ?? "static_approximate", amount, unit }, diagnostics: [] };
	return { diagnostics: [`Unable to recognize temporal expression '${text}'`] };
}

export function temporalUnitIsTimeUnit(unit: TimePrecisionLevel): boolean { return ["second", "minute", "hour", "day", "week", "month", "year", "quarter", "decade"].includes(unit); }
