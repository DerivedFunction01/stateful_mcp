...
export class SqlMacroStore implements MacroStore {
	...
	constructor(dialect: SqlDialect, private readonly executor: SqlExecutor, private readonly table = "v2_macros") {
	...
	async set(macro: V2MacroDefinition): Promise<void> {
		await this.ready;
		const query = this.compiler.upsertQuery({ macroId: macro.macroId, macroName: macro.macroName, version: macro.version, active: macro.active ? 1 : 0, status: macro.status, personnelId: macro.personnelId ?? null, profileId: macro.profileId ?? null, definition: JSON.stringify(macro) }, this.table);
	...
	private async ensureTable(): Promise<void> {
		for (const query of [...this.compiler.getTableDDL(this.table), ...this.compiler.getIndexDDL(this.table)]) await this.executor.exec(query.sql, query.params);
	}
}
