...
export class V2CellCompiler {
	...
	constructor(
		...
		profile: V2SyntaxProfile = createV2SyntaxProfile({ profileId: "v2-default" }),
	) {
		this.profile = profile;
		this.compiler = new MacroCompiler({ registry, dictionary });
	}

	async compile(rawText: string, context: V2CellCompileContext = {}): Promise<V2CellCompileResult> {
		const input = rawText.trim();
		if (!input) return { diagnostics: ["Cell input is empty"], fingerprint: fingerprint(rawText) };
		const parsed = parseMacroLine(input, 0, { profile: this.profile });
		if (!parsed) {
			if (this.profile.directCommandToken && input.startsWith(this.profile.directCommandToken))
				return { diagnostics: ["Direct commands must be executed through V2CommandBarService"], fingerprint: fingerprint(rawText) };
			return { diagnostics: ["Narrative cell compilation is not configured"], fingerprint: fingerprint(rawText) };
		}
		const definition = await this.macros.get(parsed.macroName, {
			personnelId: this.profile.personnelId,
			profileId: this.profile.profileId,
		});
		if (!definition) return { diagnostics: [`V2 macro '${parsed.macroName}' is not defined`], fingerprint: fingerprint(rawText, parsed.macroName) };
		const scope = context.documentId
			? { kind: "composite" as const, sessionId: context.sessionId ?? "", workspaceId: context.workspaceId, documentId: context.documentId }
			: context.workspaceId
			? { kind: "workspace" as const, sessionId: context.sessionId ?? "", workspaceId: context.workspaceId }
			: { kind: "clinical_document" as const, sessionId: context.sessionId ?? "" };
		const result = await this.compiler.compile(parsed, definition, { scope });
		return { plan: result.plan, diagnostics: result.diagnostics, fingerprint: result.plan?.fingerprint.value ?? fingerprint(rawText, definition.macroId) };
	}
}
