...
export class SqlTransactionJournal implements TransactionJournal {
	...
	async getByIdempotencyKey(
		...
		await this.ready;
		const query = this.compiler.getByIdempotencyQuery(
			idempotencyKey,
			this.table,
		);
		const row = await this.executor.queryOne(query.sql, query.params);
	...
	async list(query: RecoveryQuery = {}): Promise<MacroTransaction[]> {
		await this.ready;
		const compiled = this.compiler.listQuery(
			query.sourceCellId,
			query.statuses,
			this.table,
		);
	...
	private fromRow(row: Record<string, unknown>): MacroTransaction {
		if (typeof row.transactionJson === "string")
			return JSON.parse(row.transactionJson) as MacroTransaction;
