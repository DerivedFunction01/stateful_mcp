...
import {
	...
	type CommandMacroGraphPlan,
	type CommandMacroLinkOperation,
} from "./command-macro-ir";

export interface CommandMacroGraphDiagnostic {
	line?: number;
	macroName?: string;
	message: string;
}
...
function findParentForChild(
	...
	| {
			macro: ParserCommandMacro;
			plan: CommandMacroCellPlan;
			definition: NonNullable<ParserCommandMacro["children"]>[number];
	  }
	| undefined {
	for (const candidate of macros) {
		const definition = candidate.macro.children?.find(
			(item) => item.childMacroName === child.macroName,
		);
		if (definition)
			return { macro: candidate.macro, plan: candidate.plan, definition };
	}
	return undefined;
}
...
export function validateMacroCompositionGraph(
	...
	const visit = (
		...
		if (depth > maxDepth) {
			diagnostics.push(
				`${path.join(" -> ")}: composition depth exceeds ${maxDepth}`,
			);
			return;
		}
		if (path.includes(macro.macroName)) {
			diagnostics.push(
				`${[...path, macro.macroName].join(" -> ")}: cyclic child macro composition`,
			);
			return;
		}
		for (const child of macro.children ?? []) {
			...
			if (!childMacro) {
				diagnostics.push(
					`${macro.macroName}: child macro '${child.childMacroName}' is not available`,
				);
				continue;
			}
			if (
				...
				diagnostics.push(
					`${macro.macroName}: parent role '${child.parentRoleName}' is not declared by an argument`,
				);
			if (!child.parentTargetPath.trim())
				diagnostics.push(
					`${macro.macroName}: child '${child.childMacroName}' has an empty parent target path`,
				);
...
export async function planCommandMacroBatch(
	...
	diagnostics: CommandMacroGraphDiagnostic[];
}> {
	...
	for (const [lineIndex, line] of input.split(/\r?\n/).entries()) {
		...
		if (!macro) {
			diagnostics.push({
				line: lineIndex + 1,
				macroName,
				message: `unknown command macro '${macroName}'`,
			});
			continue;
		}
		...
		if (result.plan)
			resolved.push({ macro, plan: result.plan, line: lineIndex + 1 });
		if (result.plan) {
			result.plan.macroDefinitionId = macro.macroId;
			result.plan.macroDefinitionVersion = macro.version;
		}
	}
	if (diagnostics.length) return { diagnostics };
	const definitions = new Map(
		resolved.map((entry) => [entry.macro.macroName, entry.macro]),
	);
	for (const entry of resolved) {
		for (const diagnostic of validateMacroCompositionGraph(
			entry.macro,
			definitions,
			options.maxCompositionDepth,
		))
			diagnostics.push({
				line: entry.line,
				macroName: entry.macro.macroName,
				message: diagnostic,
			});
	}
	if (diagnostics.length) return { diagnostics };
	...
	for (const child of resolved) {
		...
		if (!parent) continue;
		links.push({
			linkId: `${groupId}:link:${parent.plan.cellRef}:${child.plan.cellRef}`,
			parentRef: parent.plan.cellRef,
			childRef: child.plan.cellRef,
			parentRoleName: parent.definition.parentRoleName,
			parentTargetPath: parent.definition.parentTargetPath,
			mergeStrategy: parent.definition.mergeStrategy,
			sourceLine: child.line,
		});
		child.plan.parentRef = parent.plan.cellRef;
		child.plan.linkTarget = {
			targetField: parent.definition.parentTargetPath,
			mergeStrategy: parent.definition.mergeStrategy,
		};
	}
	return {
		...
		graph: {
			...
			definitionVersions: Object.fromEntries(
				resolved.map((entry) => [entry.macro.macroId, entry.macro.version]),
			),
			diagnostics: [],
			compatibilitySignature: buildCommandMacroCompatibilitySignature(
				resolved.map((entry) => entry.plan),
				links,
			),
		},
	};
