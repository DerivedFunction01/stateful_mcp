...
export function bindCommandMacro(input: string, macro: ParserCommandMacro, options: { groupId?: string; cellRef?: string; sourceLine?: number } = {}): CommandMacroBindResult {
	const lexed = lexCommandMacro(input, macro);
	const diagnostics: CommandMacroBindDiagnostic[] = lexed.diagnostics.map((item) => ({ message: item.message }));
	...
	for (const [index, token] of lexed.arguments.entries()) {
		...
		const argument = argumentId ? macro.arguments.find((candidate) => candidate.argumentId === argumentId || candidate.name === argumentId || candidate.aliases?.includes(argumentId)) : undefined;
		if (argumentId && !argument) { diagnostics.push({ message: `unknown argument '${argumentId}'`, tokenIndex: index }); continue; }
		if (argument) {
			if (argumentsById.has(argument.argumentId) && argument.extraction.kind !== "array") diagnostics.push({ message: `duplicate scalar assignment '${argument.name}'`, argumentId: argument.argumentId, tokenIndex: index });
			argumentsById.set(argument.argumentId, token);
		} else positional.push(token);
	}
	let positionalIndex = 0;
	for (const argument of macro.arguments.slice().sort((a, b) => (a.position ?? Number.MAX_SAFE_INTEGER) - (b.position ?? Number.MAX_SAFE_INTEGER))) {
		...
		else if (argument.required && argument.blankPolicy !== "allow" && argument.blankPolicy !== "skip") diagnostics.push({ message: `required argument '${argument.name}' is missing`, argumentId: argument.argumentId });
	}
	if (positionalIndex < positional.length) diagnostics.push({ message: "too many positional arguments", tokenIndex: positionalIndex });
