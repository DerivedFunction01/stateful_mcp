...
import type { ProseTemplate, ProseSlot } from "../schemas/prose-template";
import type { ProseParserTemplateStore } from "../store/reference/prose-parser-templates/interfaces";
import type { AttributeParserRule, ConceptFieldStore, ParserSyntaxProfile } from "../store/interfaces";
...
private readonly conceptFieldStore: ConceptFieldStore,
...
const capturedText = subMatch.groups?.[slot.slotName] || subMatch.groups?.value || subText;

// Delegate parsing
if (slot.delegateTemplateId) {
const delegate = allTemplates.find((t) => t.templateId === slot.delegateTemplateId);
if (delegate) {
const { items: subItems, remnants: subRemnants } = await this.parseSection(
capturedText,
delegate,
allTemplates,
);
...
if (slot.slotType === "repeating_block" && slot.repeatPattern && slot.subTemplate) {
const flags = "gm";
...
const subItems = await this.parseSlot(repeatText, subSlot, template);
items.push(...subItems);
}
}
...
if (match) {
internalConsumed.push({ start: match.index, end: match.index + match[0].length });
}
}
}
...
const val = child.extractedData?.[targetPath] || child.concept?.[0] || child;
...
if (child.concept?.[0]) {
parent.extractedData.supportingConcepts.push(child.concept[0]);
}
}
}
}
...
const remnantText = sectionText.substring(currentPos, range.start).trim();
if (remnantText) {
remnants.push({
text: remnantText,
...
let parentId = template.parentTemplateId;
...
const hasOverride = slots.some((s) => s.slotName === parentSlot.slotName);
if (!hasOverride) {
...
const valueCapture = match.groups?.value || match.groups?.[slot.slotName] || match[0];
...
const elements: string[] = [];
		if (isList) {
			...
			elements.push(...capturedText.split(delRegex).map((s) => s.trim()).filter(Boolean));
} else {
...
this.profile.schemaNamespaces?.[targetSchema.toLowerCase()] || undefined;
...
allowedNamespaces,
				...
				const concept = resolvedConcepts.length > 0 ? [resolvedConcepts[0]!] : [];
