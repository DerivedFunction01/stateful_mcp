...
export class CommandBarService {
	...
	async preview(input: CommandBarInput): Promise<CommandPreview> {
		...
		if (
			input.rawText
				.trim()
				.startsWith(
					`${this.syntaxProfile.variableCommandToken}${this.syntaxProfile.variableCommandName}`,
				)
		) {
			try {
				...
				return {
					...
					fingerprint,
					diagnostics: variableIntent.diagnostics,
				};
			}
		}
		const plan =
			intent.operation && input.workspaceId
	...
	async execute(input: CommandExecutionInput): Promise<ExecutionResult> {
		...
		if (!preview.plan) {
			...
			return {
				...
				transactionId: "",
				planFingerprint: preview.fingerprint,
				error: "Command did not produce an executable plan",
			};
		}
		if (
			...
			return {
				...
				transactionId: "",
				planFingerprint: preview.fingerprint,
				error: "Command preview fingerprint is stale",
			};
	...
	private workspaceContext(): CommandBarWorkspaceContext {
		return {
			...
			resolveBranchRef: (workspace, ref) =>
				this.workspaceService.resolveBranchRef(workspace, ref),
		};
	}

	private async planForIntent(
		...
		const document = input.documentId
			? await this.engine.getDocument(input.documentId)
			: undefined;
		...
		return {
			...
			generatedCells: [],
			workspaceOperations: [
				intent.operation as NonNullable<
					MacroExecutionPlan["workspaceOperations"]
				>[number],
			],
			expectedVersions: [
				{
					...
					expectedHead: workspace.eventHead,
				},
				...(document
					? [
							{
