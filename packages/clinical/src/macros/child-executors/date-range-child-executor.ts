import type { ChildMacroExecutor, ChildExecutorContext, ChildExecutorResult } from "./child-executor";
import { resolveChildInputSpan } from "./child-executor";
import { matchTemporalGrammar } from "../../setup/temporal-grammar-compiler";
import { assembleClinicalDateRange, type DateRangeSlotValues } from "../../values/date-range-assembler";
...
export class DateRangeChildExecutor implements ChildMacroExecutor {
	async execute(context: ChildExecutorContext): Promise<ChildExecutorResult> {
		...
		if (!span || !span.rawValue.trim()) {
			return {
				operations: [],
				diagnostics: [`No input text found for child macro '${context.childDefinition.childMacroName}'`],
			};
		}
		...
		if (!context.compiledGrammar) {
			return {
				operations: [],
				diagnostics: [`No compiled temporal grammar provided for child macro '${context.childDefinition.childMacroName}'`],
			};
		}

		const matchResult = matchTemporalGrammar(context.compiledGrammar, span.rawValue.trim());
		if (!matchResult.match) {
			return {
				operations: [],
				diagnostics: matchResult.diagnostics,
				sourceSpan: span.start !== undefined && span.end !== undefined ? { start: span.start, end: span.end, rawValue: span.rawValue } : undefined,
			};
		}

		const parseTimestamp = (val: string) => {
			const ddmmyyyy = val.match(/^(\d{2})\.(\d{2})\.(\d{4})$/);
			if (ddmmyyyy) return `${ddmmyyyy[3]}-${ddmmyyyy[2]}-${ddmmyyyy[1]}T00:00:00.000Z`;
			return val;
		};
		...
		for (const [slotId, slotVal] of Object.entries(matchResult.match.slots)) {
			...
			if (slotId.includes("start")) {
				slots.start = { assertedTimestampUtc: parseTimestamp(slotVal), precisionLevel: "day" };
			} else if (slotId.includes("end")) {
				slots.end = { assertedTimestampUtc: parseTimestamp(slotVal), precisionLevel: "day" };
			} else if (slotId.includes("exclude")) {
				slots.excluded = [{ start: { assertedTimestampUtc: parseTimestamp(slotVal), precisionLevel: "day" } }];
			} else if (slotId.includes("include")) {
				slots.included = [{ start: { assertedTimestampUtc: parseTimestamp(slotVal), precisionLevel: "day" } }];
			}
		}
		...
		const childOperation: MacroTargetOperation = {
			...
			macroDefinitionId: context.childMacroDefinition?.macroId ?? context.childDefinition.childMacroName,
