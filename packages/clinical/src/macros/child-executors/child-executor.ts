...
import type { MacroChildDefinition, MacroDefinition } from "../macro-definition";
import type { MacroInput, MacroArgumentInput } from "../macro-binding";
...
export function resolveChildInputSpan(
	...
	if (!inputContract || inputContract.mode === "standalone") {
		if (parentInput.body) return { rawValue: parentInput.body.raw, start: parentInput.body.start, end: parentInput.body.end };
		...
		if (spaceIndex !== -1) {
			...
			if (rawValue) return { rawValue, start: spaceIndex + 1, end: fullText.length };
		}
		return undefined;
	}
	...
	if (inputContract.mode === "named") {
		const argSpec = parentDefinition.arguments.find(
			(a) => a.argumentId === inputContract.argumentId || (inputContract.aliases && inputContract.aliases.includes(a.name)),
		);
		const match = parentInput.arguments.find(
			(a) => a.name === (argSpec?.name ?? inputContract.argumentId) || (argSpec?.aliases && a.name && argSpec.aliases.includes(a.name)),
		);
		if (match) return { rawValue: match.rawValue, start: match.start, end: match.end };
		return undefined;
	}
