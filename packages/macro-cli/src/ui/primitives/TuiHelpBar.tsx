...
export function buildContextualHelpBarHints(
	...
	if (focusedPane === "modal" && layout.activeModal?.id === "settings") {
		...
		if (focusRingChord)
			hints.push({
				key: focusRingChord,
				action: translate(i18n, "command.settings.focusNext"),
				row: 1,
			});
		...
		if (sectionChord)
			hints.push({
				key: sectionChord,
				action: translate(i18n, "command.settings.nextSection"),
				row: 2,
			});
		return hints;
	}
	...
	if (focusedPane === "palette") {
		...
		if (closeChord) {
			hints.push({
				key: closeChord,
				action: translate(i18n, "helpBar.close"),
				row: 1,
			});
		}
		return hints;
	...
	if (focusedPane === "activity") {
		...
		if (
			...
			if (
				...
				return providerHints.map(
					(h: { key: string; label: string; row?: 1 | 2 }) => ({
						key: h.key,
						action: h.label,
						row: h.row ?? 1,
					}),
		...
		if (container?.contextualHints && container.contextualHints.length > 0) {
			return container.contextualHints.map(
				(h: ContextualKeyHint & { row?: 1 | 2 }) => ({
					key: h.key,
		...
		if (openChord) {
			hints.push({
				key: openChord,
				action: translate(i18n, "helpBar.open"),
				row: 1,
			});
		}
		...
		if (closeChord) {
			hints.push({
				key: closeChord,
				action: translate(i18n, "helpBar.editor"),
				row: 2,
			});
		}
	...
	if (focusedPane === "main" && layout.activeTabId !== "scratchpad") {
		...
		const bindings = surfaceKeybindingsForMode(tab?.keybindings, mode);
