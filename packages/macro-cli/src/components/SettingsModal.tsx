...
export function SettingsModal({
	...
	const modalFooter = (
		<box flexDirection="column" width={width}>
				...
				<text fg={modelSnapshot.hasErrors ? "#f87171" : "#94a3b8"}>
					{modelSnapshot.hasErrors
						...
						: modelSnapshot.totalModifiedCount > 0
							? translate(workspace.i18n, "settings.status.modified", {
									count: modelSnapshot.totalModifiedCount,
								})
			...
			<HelpBar
				keymap={
					(
						workspace.runtime as {
							context?: {
								keymap?: import("@stateful-mcp/macro").EditorKeymapProfile;
							};
						}
					).context?.keymap
				}
	...
	if (controllerSnapshot.dialog === "discard") {
		return (
			...
			>
				...
				>
					<text attributes={1} fg="#f8fafc">
						{translate(workspace.i18n, "settings.unsaved.title")}
					</text>
					<text fg="#94a3b8">
						{translate(workspace.i18n, "settings.unsaved.message")}
					</text>
					<box flexDirection="row" marginTop={1}>
						...
						<box onMouseDown={() => controller.confirmClose("discard")}>
							<TuiButton
								label={translate(workspace.i18n, "settings.discard")}
								theme={theme}
							/>
						</box>
						<box onMouseDown={() => controller.confirmClose("cancel")}>
							<TuiButton
								label={translate(workspace.i18n, "settings.keepEditing")}
								isFocused
								theme={theme}
							/>
						</box>
	...
	if (controllerSnapshot.focus === "json") {
		const lines: TextEditorLine[] = modelSnapshot.rawJsonText
			.split("\n")
			.map((text, index) => ({
				num: index + 1,
				tokens: createGenericLineParser()(text, index + 1).tokens,
				isCursorLine:
					index === modelSnapshot.rawJsonText.split("\n").length - 1,
			}));
		const cursorBefore = modelSnapshot.rawJsonText.slice(
			0,
			controllerSnapshot.jsonCursor,
		);
		const cursorLine = cursorBefore.split("\n").length;
		const cursorCol =
			(cursorBefore.lastIndexOf("\n") < 0
				? cursorBefore.length
		...
		>
			...
			>
				<SettingsWindowView
					...
					i18n={workspace.i18n}
					focusedRegion={
						controllerSnapshot.focus === "categories"
							? "navigation"
							: controllerSnapshot.focus === "profile" ||
									controllerSnapshot.focus === "scope"
								? "search"
								: controllerSnapshot.focus === "actions"
									? "content"
									: controllerSnapshot.focus
					}
					selectedCategoryId={
						modelSnapshot.sections[controllerSnapshot.selectedCategoryIndex]?.id
					}
