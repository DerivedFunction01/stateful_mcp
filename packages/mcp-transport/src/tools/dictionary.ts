...
function registerAllTools(
	...
	const workspaceSchema =
		exposeWorkspace && workspaces.length > 0
			? z
					.enum(workspaces as [string, ...string[]])
					.optional()
					.describe("The target workspace isolation context.")
			: undefined;

	const tagEnumSchema =
		exposeTags && allowedTags.length > 0
	...
	const tagsSchema = z
		.array(tagEnumSchema)
		.optional()
		.describe("Tags classifying the entry.");
		...
		"dictionary_add_concept",
		{
			...
			inputSchema: {
				...
				namespace_code: z
					...
					.optional()
					.describe(
						"Target namespace code. Defaults to config dynamic default.",
					),
				description: z
					.string()
					.optional()
					.describe("Description of the concept."),
			},
		},
			...
			try {
				...
				return {
					content: [
						{
							type: "text" as const,
							text: JSON.stringify({ concept_id: conceptId }),
						},
					],
				};
			} catch (err: any) {
	...
	server.registerTool(
		"dictionary_add_relation",
		{
			...
			inputSchema: {
				source_concept_ref: z
					.string()
					.describe(
						"Source concept reference (UUID or 'NAMESPACE::CODE' coordinate).",
					),
				target_concept_ref: z
					.string()
					.describe(
						"Target concept reference (UUID or 'NAMESPACE::CODE' coordinate).",
					),
				relationship_type: z
					.enum(["EQUIVALENT", "NARROWER_THAN", "WIDER_THAN"])
		...
		async ({ source_concept_ref, target_concept_ref, relationship_type }) => {
			try {
				...
				if (!sourceId)
					throw new Error(
						`Source concept reference "${source_concept_ref}" not found.`,
					);
				const targetId = await store.resolveConceptId(target_concept_ref);
				if (!targetId)
					throw new Error(
						`Target concept reference "${target_concept_ref}" not found.`,
					);
				...
				return {
					content: [
						{
							type: "text" as const,
							text: JSON.stringify({ relation_id: relationId }),
						},
					],
				};
			} catch (err: any) {
	...
	const addExpressionSchema: any = {
		...
		regex_pattern: z
			...
			.optional()
			.describe(
				"Custom regex pattern evaluated by NodeJS RegExp. Do NOT wrap in slashes /.../ or append flags. Default matches term.",
			),
		is_case_insensitive: z
			.boolean()
			.optional()
			.default(true)
			.describe(
				"Whether matching is case-insensitive. Configures the JS RegExp 'i' flag under the hood.",
			),
		tags: tagsSchema,
		description: z
			.string()
			.optional()
			.describe("Context/reason for the entry mapping."),
		user_id: z.string().optional().describe("User ID of the caller."),
		is_admin: z
			.boolean()
			.optional()
			.describe("Whether the caller has administrative privileges."),
	};
	...
	server.registerTool(
		...
		{
			description:
				"Register a custom shorthand expression mapping to a concept.",
			...
			try {
				...
				return {
					content: [
						{
							type: "text" as const,
							text: JSON.stringify({ dict_entry_id: entryId }),
						},
					],
				};
			} catch (err: any) {
	...
	const editExpressionSchema: any = {
		...
		regex_pattern: z
			...
			.optional()
			.describe(
				"New custom regex pattern evaluated by NodeJS RegExp. Do NOT wrap in slashes /.../ or append flags.",
			),
		is_case_insensitive: z
			.boolean()
			.optional()
			.describe(
				"New case insensitivity setting configuring the JS RegExp 'i' flag.",
			),
		tags: tagsSchema,
		description: z
			.string()
			.optional()
			.describe("New context/reason for the entry mapping."),
		user_id: z.string().optional().describe("User ID of the caller."),
		is_admin: z
			.boolean()
			.optional()
			...
			try {
				...
				return {
					content: [
						{
							type: "text" as const,
							text: JSON.stringify({ dict_entry_id, success: true }),
						},
					],
				};
			} catch (err: any) {
	...
	server.registerTool(
		...
		{
			description:
				"Edit properties of an existing concept (display or description).",
			inputSchema: {
				concept_ref: z
					.string()
					.describe(
				...
				display: z
					.string()
					.optional()
					.describe("New display name for the concept."),
				description: z
					.string()
					.optional()
					.describe("New description for the concept."),
			},
		},
			...
			try {
				...
				return {
					content: [
						{
							type: "text" as const,
							text: JSON.stringify({ concept_id: conceptId, success: true }),
						},
					],
				};
			} catch (err: any) {
		...
		"dictionary_remove_concept",
		{
			...
			inputSchema: {
				concept_ref: z
					.string()
					.describe(
						"Concept reference (UUID or 'NAMESPACE::CODE' coordinate) to deactivate.",
					),
			},
		},
		async ({ concept_ref }) => {
			...
			try {
				...
				return {
					content: [
						{
							type: "text" as const,
							text: JSON.stringify({ relation_id, deactivated: true }),
						},
					],
				};
			} catch (err: any) {
	...
	server.registerTool(
		...
		async (args: any) => {
			try {
				...
				return {
					content: [
						{ type: "text" as const, text: JSON.stringify({ resolved: res }) },
					],
				};
			} catch (err: any) {
				return {
	...
	server.registerTool(
		...
		{
			description:
				"Search expression mappings by term, tags, concept type, or workspace",
	...
	server.registerTool(
		...
		{
			description:
				"Remove an expression entry by ID (soft-delete if resolved metrics exist, else hard-delete).",
			inputSchema: {
				dict_entry_id: z
					.string()
					.describe("ID of the expression entry to delete."),
				user_id: z.string().optional().describe("User ID of the caller."),
				workspace_id: z
					.string()
					.optional()
					.describe("Workspace ID of the caller."),
				is_admin: z
					.boolean()
					.optional()
					.describe("Whether the caller has administrative privileges."),
		...
		async ({ dict_entry_id, user_id, workspace_id, is_admin }) => {
			try {
				const ws = workspace_id || store.getDefaultWorkspace();
				const callerContext = { user_id, workspace_id: ws, is_admin };
				const removed = await store.removeExpression(
					dict_entry_id,
					callerContext,
				);
				return {
	...
	server.registerTool(
		...
		{
			description:
				"Get meta-documentation explaining how to resolve and utilize terminology optimal for LLM context windows",
			inputSchema: {},
		},
		async () => {
			try {
	...
	server.registerTool(
		...
		{
			description:
				"Get worked conversation transcript examples showing ideal multi-turn interaction with the dictionary service",
			inputSchema: {
...
async function main() {
	...
	if (config.dictionary_state) {
		try {
			const dictionaryConfig = (await resolveSource(
				config.dictionary_state,
				workspaceRoot,
			)) as any;
			if (dictionaryConfig) {
