...
import { z } from "zod";
import type { FilterStore, ObjectStore, FormStore, EventStore, MiddlewareConfig } from "@stateful-mcp/core";
import { getFilterStore, getObjectStore, getFormStore, getEventStore } from "./helper.js";
...
export async function registerStateInitTool(
  ...
  if (needed.size > 0) {
    ...
    if (needed.has("filter")) {
      results.filter = await probeStore(() => filterStore.getFilter(probeSession, probeSession));
    }
    if (needed.has("object")) {
      results.object = await probeStore(() => objectStore.getObject(probeSession, probeSession));
    }
    if (needed.has("form")) {
      results.form = await probeStore(() => formStore.getForm(probeSession, probeSession));
    }
    if (needed.has("event")) {
      results.event = await probeStore(() => eventStore.getCommit(probeSession, probeSession));
    }
    ...
    console.error(`[state_init] Startup probe passed for stores: ${[...needed].join(", ")}`);
  }
  ...
  server.registerTool(
    "state_init",
    {
      ...
      inputSchema: {
        tool_name: z
          .string()
          .describe("The name of the tool whose state_requirements should be initialized."),
        suffix: z
          .string()
          .optional()
          .describe(
    ...
    async ({ tool_name, suffix }, extra: any) => {
        ...
        for (const req of requirements) {
          ...
          if (baseAlias) {
            if (suffix) {
              ...
              let counter = 1;
              while (true) {
                const candidate = counter === 1 ? baseAlias : `${baseAlias}_${counter}`;
                const taken = await aliasExists(req.type, candidate, session_id, user_id);
                if (!taken) { finalAlias = candidate; break; }
                counter++;
              }
            }
          }
          ...
          if (finalAlias && suffix) {
            const existing = await getExistingId(req.type, finalAlias, session_id, user_id);
            if (existing) {
          ...
          if (req.type === "filter") {
            await filterStore.init(session_id, req.toolName, req.tableName, user_id, finalAlias);
          ...
          bucket(results, req.type)[req.id] = finalAlias ?? `<id assigned by ${req.type}>`;
        }
      ...
      async function aliasExists(type: StoreType, alias: string, sid: string, uid?: string): Promise<boolean> {
        return (await getExistingId(type, alias, sid, uid)) !== null;
      }

      async function getExistingId(type: StoreType, alias: string, sid: string, uid?: string): Promise<string | null> {
        if (type === "filter") { const s = await filterStore.getFilter(alias, sid, uid); return s?.filterId ?? null; }
        if (type === "object") { const s = await objectStore.getObject(alias, sid, uid); return s?.objectId ?? null; }
        if (type === "form")   { const s = await formStore.getForm(alias, sid);           return s?.formId   ?? null; }
        if (type === "event")  { const s = await eventStore.getCommit(alias, sid);        return s?.commitId ?? null; }
        return null;
      }
    }
  );
...
function bucket(
  results: { filters: Record<string, string>; objects: Record<string, string>; forms: Record<string, string>; events: Record<string, string> },
  type: string
