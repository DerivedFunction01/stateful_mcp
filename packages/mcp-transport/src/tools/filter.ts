...
function registerFilterTools(
	...
	const filterConditionSchema = z.object({
		...
		operator: z
			.enum([
				...
				"gt", // < <= >= >  (numeric / date)
				"like",
				"not_like", // raw LIKE pattern (caller supplies %)
				"starts_with",
				"ends_with",
				"str_contains", // LIKE 'x%' '%x' '%x%'  (no %% needed)
				"in_set",
				"not_in_set", // IN (...) NOT IN (...)
				"between",
				"not_between", // BETWEEN a AND b
			])
			.describe(
	...
	const aggregationSchema = z.object({
		...
		property: z
			.string()
			.describe("The property to aggregate (or '*' for count)."),
		...
		"filter_init",
		{
			...
			inputSchema: {
				tool_name: z
					.string()
					.optional()
					.describe("Optional target tool for schema binding."),
				table_name: z
					.string()
					.optional()
					.describe("Optional sub-table within the tool."),
				alias: z
					.string()
					.optional()
					.describe("Optional descriptive alias to tag the initial state."),
				rules: z
					.array(filterConditionSchema)
					.optional()
					.describe("Optional initial list of filter rules to apply."),
			},
		},
		async ({ tool_name, table_name, alias, rules }, extra: any) => {
			const session_id = extra?._metadata?.session_id ?? "default";
			try {
				const filterId = await filterStore.init(
					session_id,
					tool_name,
					table_name,
					undefined,
				...
				return {
					content: [
						{ type: "text", text: JSON.stringify({ filter_id: filterId }) },
					],
				};
			} catch (err: any) {
				return {
	...
	server.registerTool(
		"filter_add",
		{
			...
			inputSchema: {
				...
				operations: z
					.array(filterConditionSchema)
					.describe("List of filter conditions to add."),
				new_alias: z
					.string()
					.optional()
					.describe(
						"Optional new descriptive alias to point to the mutated head, leaving the old alias at the parent checkpoint.",
					),
			},
		},
		async ({ filter_id, operations, new_alias }, extra: any) => {
			...
			try {
				const newFilterId = await filterStore.add(
					filter_id,
					operations as any,
					session_id,
					user_id,
				...
				return {
					content: [
						{
							type: "text",
							text: JSON.stringify({ new_filter_id: newFilterId }),
						},
					],
				};
			} catch (err: any) {
	...
	server.registerTool(
		...
		async ({ filter_id }, extra: any) => {
			...
			const user_id = extra?._metadata?.user_id;
			try {
				const filter = await filterStore.getFilter(
					filter_id,
					session_id,
					user_id,
				);
				if (!filter) {
					return {
						content: [
							{ type: "text", text: `Filter ID "${filter_id}" not found.` },
						],
						isError: true,
					};
				}
			...
			try {
				...
				return {
					content: [
						{
							type: "text",
							text: JSON.stringify({ compressed_filter_id: compressedId }),
						},
					],
				};
			} catch (err: any) {
	...
	server.registerTool(
		"filter_combine",
		{
			...
			inputSchema: {
				...
				ids: z
					.array(z.string())
					.describe("List of filter or view IDs to combine."),
			},
		},
		async ({ operation, ids }, extra: any) => {
			...
			try {
				...
				return {
					content: [
						{ type: "text", text: JSON.stringify({ combined_id: combinedId }) },
					],
				};
			} catch (err: any) {
				return {
			...
			try {
				const ownerScope =
					scope === "global"
						...
						: { level: "user" as const, userId: user_id || "" };
		...
		"filter_parameters",
		{
			...
			inputSchema: {
				...
				table_name: z
					.string()
					.optional()
					.describe("Specify a sub-table within the tool."),
			},
		},
				...
				if (!params) {
					return {
						content: [
							{
								type: "text",
								text: `Tool "${tool_name}" not found in registered schemas.`,
							},
						],
						isError: true,
					};
		...
		"filter_init_modifier",
		{
			...
			inputSchema: {
				filter_id: z
					.string()
					.optional()
					.describe("Optional filter to apply modifier to"),
				columns: z.array(z.string()).optional().describe("Columns to group by"),
				aggregations: z
					.array(aggregationSchema)
					.optional()
					.describe("Mathematical roll-ups to apply to grouped data"),
			},
		},
		async ({ filter_id, columns, aggregations }) => {
			try {
				const modId = filterStore.initModifier(
					filter_id,
					columns,
					aggregations,
				);
				return {
					content: [{ type: "text", text: JSON.stringify({ mod_id: modId }) }],
				};
			} catch (err: any) {
				return {
					content: [{ type: "text", text: err.message || String(err) }],
	...
	server.registerTool(
		...
		async ({ mod_id, columns, aggregations }) => {
			try {
				...
				return {
					content: [
						{ type: "text", text: JSON.stringify({ new_mod_id: newModId }) },
					],
				};
			} catch (err: any) {
				return {
	...
	server.registerTool(
		"filter_init_view",
		{
			...
			inputSchema: {
				...
				having_id: z
					.string()
					.optional()
					.describe("Which having clause to apply"),
		...
		async ({ filter_id, mod_id, having_id, limit, offset }) => {
			try {
				const viewId = filterStore.initView(
					filter_id,
					mod_id,
					having_id,
					limit,
				...
				return {
					content: [
						{ type: "text", text: JSON.stringify({ view_id: viewId }) },
					],
				};
			} catch (err: any) {
				return {
	...
	server.registerTool(
		...
		{
			description:
				"Remove specific query conditions by property name and operator, returning a new filter state ID",
			inputSchema: {
		...
		async ({ filter_id, property, operator }, extra: any) => {
			...
			const user_id = extra?._metadata?.user_id;
			try {
				const newFilterId = await filterStore.removeRule(
					filter_id,
					property,
					operator,
					session_id,
	...
	server.registerTool(
		...
		{
			description:
				"Compare two filter states in the session to find added/removed rules",
			inputSchema: {
				filter_id_a: z.string().describe("The first filter state ID or alias."),
				filter_id_b: z
					.string()
					.describe("The second filter state ID or alias."),
			},
		},
		async ({ filter_id_a, filter_id_b }, extra: any) => {
			...
			const user_id = extra?._metadata?.user_id;
			try {
				const diffResult = await filterStore.diff(
					filter_id_a,
					filter_id_b,
					session_id,
					user_id,
				...
				return {
					content: [
						{ type: "text", text: JSON.stringify(diffResult, null, 2) },
					],
				};
			} catch (err: any) {
				return {
	...
	server.registerTool(
		...
		{
			description:
				"Tag an existing filter checkpoint with a new descriptive alias",
			inputSchema: {
		...
		async ({ id_or_alias, alias }, extra: any) => {
			const session_id = extra?._metadata?.session_id ?? "default";
			try {
				const resolved = await filterStore["resolveId"](
					id_or_alias,
					session_id,
				);
	...
	server.registerTool(
		...
		{
			description:
				"Prune intermediate filter checkpoints in the current session that are not in the ancestry of the specified active/keep filters or active aliases",
			inputSchema: {
				keep: z
					.array(z.string())
					.describe(
						"Ancestors of these filter IDs or aliases will be preserved.",
					),
				confirm: z
					.boolean()
					.optional()
					.describe("Explicit confirmation required if keep array is empty."),
				keep_aliases: z
					.array(z.string())
					.optional()
					.describe("Whitelist: only keep these aliases (delete all others)."),
				delete_aliases: z
					.array(z.string())
					.optional()
					.describe("Blacklist: explicitly delete these aliases."),
			},
		},
		async ({ keep, confirm, keep_aliases, delete_aliases }, extra: any) => {
			...
			try {
				...
				const result = await filterStore.gc(
					session_id,
					keep,
					keep_aliases,
					delete_aliases,
	...
	server.registerTool(
		...
		{
			description:
				"Get meta-documentation explaining how to design and compose stateful filter queries optimal for LLM context windows",
			inputSchema: {},
		},
		async () => {
			try {
	...
	server.registerTool(
		...
		{
			description:
				"Get worked conversation transcript examples showing ideal multi-turn interaction with the stateful filter service",
			inputSchema: {
