...
function registerEventTools(
	...
	server.registerTool(
		"event_init",
		{
			...
			inputSchema: {
				schema_name: z
					.string()
					.describe("The name of the registered event schema definition."),
				alias: z
					.string()
					.optional()
					.describe(
						"Optional descriptive alias to tag the initial checkpoint.",
					),
				data: z
					.array(z.record(z.string(), z.any()))
					.optional()
					.describe("Optional initial list of event records to pre-populate."),
				get_schema_hint: z
					.boolean()
					.optional()
					.describe(
						"If true, returns the JSON schema definition in the response to guide parameter population.",
					),
			},
		},
		async ({ schema_name, alias, data, get_schema_hint }, extra: any) => {
			...
			try {
				const commitId = await eventStore.init(
					schema_name,
					session_id,
					alias,
					data,
	...
	server.registerTool(
		"event_append",
		{
			...
			inputSchema: {
				id_or_alias: z
					.string()
					.describe("The tip commit ID or alias being appended to."),
				data: z
					.record(z.string(), z.any())
					.describe("Event parameter values satisfying the schema."),
				alias: z
					.string()
					.optional()
					.describe("Optional alias to tag the new tip checkpoint."),
			},
		},
		async ({ id_or_alias, data, alias }, extra: any) => {
			const session_id = extra?._metadata?.session_id ?? "default";
			try {
				const commitId = await eventStore.append(
					session_id,
					id_or_alias,
					data,
					alias,
				...
				return {
					content: [
						{ type: "text", text: JSON.stringify({ commit_id: commitId }) },
					],
				};
			} catch (err: any) {
				return {
	...
	server.registerTool(
		...
		{
			description:
				"Patch an existing event instance within the log using a sparse delta",
			inputSchema: {
				id_or_alias: z
					.string()
					.describe("The tip commit ID or alias to patch."),
				event_id: z
					.string()
					.describe("The unique event ID of the record in the array."),
				patch_data: z
					.record(z.string(), z.any())
					.describe("Sparse parameter key-value pairs to update."),
				alias: z
					.string()
					.optional()
					.describe("Optional alias to tag the new checkpoint."),
			},
		},
		async ({ id_or_alias, event_id, patch_data, alias }, extra: any) => {
			const session_id = extra?._metadata?.session_id ?? "default";
			try {
				const commitId = await eventStore.patch(
					session_id,
					id_or_alias,
					event_id,
					patch_data,
	...
	server.registerTool(
		"event_delete",
		{
			...
			inputSchema: {
				id_or_alias: z
					.string()
					.describe("The tip commit ID or alias to delete from."),
				event_id: z
					.string()
					.describe("The unique event ID of the record to remove."),
				alias: z
					.string()
					.optional()
					.describe("Optional alias to tag the new checkpoint."),
		...
		async ({ id_or_alias, event_id, alias }, extra: any) => {
			const session_id = extra?._metadata?.session_id ?? "default";
			try {
				const commitId = await eventStore.delete(
					session_id,
					id_or_alias,
					event_id,
					alias,
	...
	server.registerTool(
		...
		{
			description:
				"Merge multiple parallel commit streams into a target commit. On conflict, returns summary counts only — use event_merge_inspect to page through individual conflicts.",
			inputSchema: {
				source_ids_or_aliases: z
					.array(z.string())
					.describe("List of source branches or commit IDs to merge in."),
				target_id_or_alias: z
					.string()
					.describe("The target branch or commit ID receiving the merge."),
			},
		},
		async ({ source_ids_or_aliases, target_id_or_alias }, extra: any) => {
			const session_id = extra?._metadata?.session_id ?? "default";
			try {
				const res = await eventStore.merge(
					session_id,
					source_ids_or_aliases,
					target_id_or_alias,
				);
				if (res.status === "conflict") {
					const total = res.conflicts?.length ?? 0;
					const pending =
						res.conflicts?.filter((c: any) => c.status === "pending").length ??
						total;
					return {
						content: [
							{
	...
	server.registerTool(
		...
		{
			description:
				"Inspect the status of a stateful merge resolution session, paging through conflicts",
			inputSchema: {
				merge_session_id: z
					.string()
					.describe("The identifier of the merge session."),
				offset: z
					.number()
					.optional()
					.describe(
						"Zero-based index of the first conflict to return (default 0).",
					),
	...
	server.registerTool(
		"event_merge_resolve",
		{
			...
			inputSchema: {
				merge_session_id: z
					.string()
					.describe("The current merge session identifier."),
				event_id: z.string().describe("The event ID with the conflict."),
				resolution: z
					.object({
						strategy: z
							.enum(["accept_source", "accept_target", "patch"])
							.describe("Strategy resolution."),
						source_id: z
							.string()
							.optional()
							.describe(
								"Required if accept_source, identifying which source branch value to keep.",
							),
						values: z
							.record(z.string(), z.any())
							.optional()
							.describe(
								"Required if patch, supplying custom merge parameter values.",
							),
					})
		...
		async ({ merge_session_id, event_id, resolution }) => {
			try {
				const newSessionId = await eventStore.mergeResolve(
					merge_session_id,
					event_id,
					resolution,
				);
				return {
					content: [
						{
							type: "text",
							text: JSON.stringify({ merge_session_id: newSessionId }),
						},
					],
				};
			} catch (err: any) {
	...
	server.registerTool(
		...
		{
			description:
				"Finalize and commit a merge session after resolving all conflicts",
			inputSchema: {
				merge_session_id: z
					.string()
					.describe("The resolved merge session identifier."),
			},
		},
		async ({ merge_session_id }, extra: any) => {
			const session_id = extra?._metadata?.session_id ?? "default";
			try {
				const commitId = await eventStore.mergeCommit(
					merge_session_id,
					session_id,
				);
				return {
	...
	server.registerTool(
		...
		{
			description:
				"Get meta-documentation explaining how to compose and merge stateful event log streams optimal for LLM context windows",
			inputSchema: {},
		},
		async () => {
			try {
	...
	server.registerTool(
		...
		{
			description:
				"Get worked conversation transcript examples showing ideal multi-turn interaction with the stateful event service",
			inputSchema: {
