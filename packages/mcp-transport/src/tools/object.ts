...
function registerObjectTools(
	...
	server.registerTool(
		"object_init",
		{
			...
			inputSchema: {
				...
				data: z
					...
					.optional()
					.describe("Optional initial key-value data to populate."),
				get_schema_hint: z
					.boolean()
					.optional()
					.describe(
		...
		async ({ schema_name, alias, data, get_schema_hint }, extra: any) => {
			...
			try {
				const objectId = await objectStore.init(
					schema_name,
					session_id,
					alias,
					data,
	...
	server.registerTool(
		"object_set",
		{
			...
			inputSchema: {
				object_id: z
					.string()
					.describe("The target object version ID or alias."),
				path: z
					.array(pathSegmentSchema)
					.describe("Field path (e.g. ['date_range', 'start_date'])."),
			...
			try {
				...
				return {
					content: [
						{
							type: "text",
							text: JSON.stringify({ new_object_id: newObjectId }),
						},
					],
				};
			} catch (err: any) {
	...
	server.registerTool(
		"object_patch",
		{
			...
			inputSchema: {
				...
				partial: z
					.record(z.string(), z.any())
					.describe("Sparse properties delta overlay."),
			},
		},
		async ({ object_id, partial }, extra: any) => {
			...
			const user_id = extra?._metadata?.user_id;
			try {
				const newObjectId = await objectStore.patch(
					object_id,
					partial,
					session_id,
					user_id,
	...
	server.registerTool(
		"object_ref",
		{
			...
			inputSchema: {
				...
				path: z
					.array(pathSegmentSchema)
					.describe("The path where reference will reside."),
				source_object_id: z
					.string()
					.describe("The referenced source object ID."),
				source_path: z
					.array(pathSegmentSchema)
	...
	server.registerTool(
		...
		{
			description:
				"Perform recursively completeness and cross-field constraint validations",
			inputSchema: {
		...
		async ({ object_id }, extra: any) => {
			...
			const user_id = extra?._metadata?.user_id;
			try {
				const result = await objectStore.validate(
					object_id,
					session_id,
					user_id,
				);
	...
	server.registerTool(
		...
		{
			description:
				"Materialize final object payload, verifying validation holds",
			inputSchema: {
				object_id: z.string().describe("The object ID to resolve."),
				mode: z
					.enum(["tool_call", "function"])
					.describe("Target payload format."),
			},
		},
		async ({ object_id, mode }, extra: any) => {
			...
			const user_id = extra?._metadata?.user_id;
			try {
				const payload = await objectStore.resolve(
					object_id,
					mode,
					session_id,
					user_id,
	...
	server.registerTool(
		...
		{
			description:
				"Perform an operation (insert, remove, replace) on an array field",
			inputSchema: {
				object_id: z.string().describe("The object ID."),
				path: z.array(pathSegmentSchema).describe("Field path to the array."),
				operation: z
					.enum(["insert", "remove", "replace"])
					.describe("The operation to perform."),
				index: z
					.number()
					.optional()
					.describe(
						"Target position. Required for remove/replace; optional for insert.",
					),
				value: z
					.any()
					.optional()
					.describe(
						"Value payload. Required for replace; optional for insert.",
					),
			},
		},
		async (
	...
	server.registerTool(
		...
		async ({ object_id }, extra: any) => {
			...
			const user_id = extra?._metadata?.user_id;
			try {
				const compressedId = await objectStore.compress(
					object_id,
					session_id,
					user_id,
				);
				return {
					content: [
						{
							type: "text",
							text: JSON.stringify({ compressed_object_id: compressedId }),
						},
					],
				};
			} catch (err: any) {
	...
	server.registerTool(
		...
		async ({ object_id, tags, description, scope }, extra: any) => {
			...
			try {
				...
				const savedId = await objectStore.save(
					object_id,
					tags,
					description,
					ownerScope,
	...
	server.registerTool(
		...
		async ({ object_id }, extra: any) => {
			...
			const user_id = extra?._metadata?.user_id;
			try {
				const inspect = await objectStore.inspect(
					object_id,
					session_id,
					user_id,
				);
				return {
					content: [{ type: "text", text: JSON.stringify(inspect, null, 2) }],
				};
			} catch (err: any) {
				return {
					content: [{ type: "text", text: err.message || String(err) }],
	...
	server.registerTool(
		...
		async ({ object_id_a, object_id_b }, extra: any) => {
			...
			const user_id = extra?._metadata?.user_id;
			try {
				const diff = await objectStore.diff(
					object_id_a,
					object_id_b,
					session_id,
					user_id,
	...
	server.registerTool(
		...
		{
			description:
				"Tag an existing object checkpoint with a new descriptive alias",
			inputSchema: {
		...
		async ({ id_or_alias, alias }, extra: any) => {
			const session_id = extra?._metadata?.session_id ?? "default";
			try {
				const resolved = await objectStore["resolveId"](
					id_or_alias,
					session_id,
				);
	...
	server.registerTool(
		...
		{
			description:
				"Prune intermediate object checkpoints in the current session that are not in the ancestry of the specified active/keep objects or active aliases",
			inputSchema: {
				keep: z
					.array(z.string())
					.describe(
						"Ancestors of these object IDs or aliases will be preserved.",
					),
				confirm: z
					.boolean()
					.optional()
		...
		async ({ keep, confirm, keep_aliases, delete_aliases }, extra: any) => {
			...
			try {
				...
				const result = await objectStore.gc(
					session_id,
					keep,
					keep_aliases,
					delete_aliases,
	...
	server.registerTool(
		...
		{
			description:
				"Get meta-documentation explaining how to validate and compose stateful objects optimal for LLM context windows",
			inputSchema: {},
		},
		async () => {
			try {
				...
				const content = await resolveAboutOrExamples(
					...
					path.join(localAboutDir, "object.md"),
					workspaceRoot,
				);
	...
	(server as any).registerTool(
		...
		{
			description:
				"Get worked conversation transcript examples showing ideal multi-turn interaction with the stateful object service",
			inputSchema: {
		...
		async ({ page, limit }: { page?: number; limit?: number }) => {
			try {
				...
				let content = await resolveAboutOrExamples(
					...
					path.join(localExamplesDir, "object.md"),
					workspaceRoot,
				);
...
async function main() {
	...
	objectStore = await getObjectStore(config, workspaceRoot);

	const [filterStore, formStore] = await Promise.all([
